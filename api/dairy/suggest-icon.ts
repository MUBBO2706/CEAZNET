  ...
  try {
    ...
    const resultText = await executeWithGeminiRotation("suggest_dairy_icon", async (ai) => {
      const response = await ai.models.generateContent({
        ...
        contents: `Suggest the single best icon ID for a daily ledger/diary tracker item named "${name}".
...
Return your response strictly in JSON format matching this structure:
{"icon": string, "confidence": number, "reason": string}.
Example: {"icon": "milk", "confidence": 0.95, "reason": "Dudh refers to milk in Hindi"}`,
        config: {
          tools: [{ googleSearch: {} }],
          temperature: 0.2,
        }
    ...
    if (resultText) {
      ...
      return res.json({ success: true, icon: parsed.icon, confidence: parsed.confidence, reason: parsed.reason });
    } else {
