...
async function getValidImageCache() {
  ...
  if (changed && !isPruningCache) {
    ...
    saveImageCacheUnified(cache, { totalSent: 0, processedCount: 0, cachedCount: Object.keys(cache).filter(k => k !== '__stats__').length, failedCount: 0 })
      .catch(err => console.warn('[Image Cache] Pruning save error:', err))
      .finally(() => {
  ...
  if (base64) {
    ...
    if (!skipSave) {
      await saveImageCacheUnified(imageCache, {
        ...
        cachedCount: 0,
        failedCount: 0
      }).catch(err => console.warn('[Image Cache] Error saving single image cache:', err));
    }
    return {
      dataUrl,
      status: 'processed',
    ...
    if (req.method === 'POST') {
        ...
        await Promise.all(chunk.map(async (url) => {
          try {
            ...
            if (detail.status === 'failed') {
              if (shouldReturnData) resultsData[url] = "failed";
            } else if (detail.dataUrl && shouldReturnData) {
      ...
      if (needsSave || processedCount > 0) {
        try {
          ...
          console.warn('[Image Proxy] Failed to save image cache to Telegram:', e.message || e);
        }
      }
