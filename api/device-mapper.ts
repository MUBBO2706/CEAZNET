...
const supabaseAdminKey = process.env.SUPABASE_SERVICE_ROLE_KEY || process.env.SUPABASE_KEY || process.env.VITE_SUPABASE_ANON_KEY || 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Iml0anVyZ3FidnNxbmlwaHVlaGl6Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzUyODM5NTgsImV4cCI6MjA5MDg1OTk1OH0.WSyZbgJ7rcbaTGCwURHTxQCHU9__F_ql75L6upVsVag';
...
export default async function handler(req: any, res: any) {
  ...
  if (action === 'clear_audit_logs' && req.method === 'POST') {
    try {
      ...
      try {
        ...
        console.warn("Failed to clear DB audit logs:", dbErr);
      }
  ...
  // Consolidated endpoint for Devices Tab initialization
  if (action === 'init_devices_tab' && req.method === 'GET') {
    try {
      const data = await loadDeviceCacheUnified();
      const model = await loadPreferredAiModel();
      return sendJson(200, { cache: data, aiModel: model }, { action: 'init_devices_tab' });
    } catch (err: any) {
      console.error("Error init_devices_tab:", err.message);
      return sendJson(500, { error: err.message }, { action: 'init_devices_tab' });
    }
  }
