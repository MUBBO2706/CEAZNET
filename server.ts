...
export const sseClients: { id: string; res: any }[] = [];
export const sessionSseClients: { id: string; res: any }[] = [];

// Keep original consoles intact
const originalConsoleLog = console.log;
const originalConsoleError = console.error;
const originalConsoleWarn = console.warn;
const originalConsoleInfo = console.info;

function addServerLog(type: 'log' | 'info' | 'warn' | 'error', ...args: any[]) {
  ...
  let logType = type;
  if (logType === 'error' && typeof message === 'string') {
    if (message.includes('[Image-Proxy] Batch complete') || message.includes('Successfully resolved image') || message.includes('[Image-Proxy] Successfully resolved')) {
      logType = 'log';
    }
  }

  const newLogEntry: ServerLogEntry = {
  ...
  // Broadcast to all connected Server-Sent Events (SSE) clients in real-time
  const sseData = `data: ${JSON.stringify(newLogEntry)}\n\n`;
  sseClients.forEach(client => {
    try {
      client.res.write(sseData);
    } catch (e) {
      // client res might be closed, handled in 'close' event trigger
    }
  });
}

// Override console methods to capture server-side logs
console.log = (...args: any[]) => {
  originalConsoleLog(...args);
  addServerLog('log', ...args);
};
...
process.on('uncaughtException', (err) => {
  originalConsoleError('[CRASH PREVENTED] Uncaught Exception:', err);
  addServerLog('error', `[CRASH PREVENTED] Uncaught Exception: ${err instanceof Error ? err.stack || err.message : String(err)}`);
});

process.on('unhandledRejection', (reason, promise) => {
  originalConsoleError('[CRASH PREVENTED] Unhandled Rejection:', reason);
  addServerLog('error', `[CRASH PREVENTED] Unhandled Rejection: ${reason instanceof Error ? reason.stack || reason.message : String(reason)}`);
});
...
async function startServer() {
  ...
  async function resolveImageUrlDetailed(url: string): Promise<ImageResolutionResult> {
    ...
    if (base64) {
      console.log(`[Image-Proxy] Successfully resolved image via ${successfulStrategy}: ${url.slice(0, 60)}${url.length > 60 ? '...' : ''}`);
    ...
    console.warn(`[Image-Proxy] Failed to resolve image after ${attempts} attempts: ${url.slice(0, 60)}${url.length > 60 ? '...' : ''}. Error: ${finalError || 'Unknown failure'}`);
  ...
  app.post("/api/image-proxy", async (req, res) => {
    try {
      ...
      console.log(`[Image-Proxy] Starting batch image resolution for ${urls.length} URLs...`);
      ...
      const fetchPromises = urls.map(async (url) => {
        ...
        try {
          ...
          if (detail.status !== 'failed' && detail.dataUrl) {
      ...
      const serverISTTime = new Date().toLocaleString('en-IN', { timeZone: 'Asia/Kolkata', hour12: true });

      console.log(`[Image-Proxy] Batch complete. Sent: ${urls.length} | Cached: ${cachedCount} | Processed: ${processedCount} | Failed: ${failedCount} (IST: ${serverISTTime})`);
  ...
  // Server-side logs access routes
  ...
  app.get("/api/server-logs/stream", (req, res) => {
    try {
      ...
      res.setHeader('X-Accel-Buffering', 'no'); // Prevent buffering in proxy layers (like nginx)

      // Send a dummy comment to verify connection and flush headers
      res.write(': ping\n\n');

      // Send the currently stored log history using an "initial" event type
      ...
      console.error("[SSE server.ts] SSE Stream bootstrap failed:", e);
      res.status(500).end();
    }
  });
  ...
  app.post("/api/server-logs-clear", (req, res) => {
    try {
      serverLogs.length = 0;
      console.log("[Server Logs] Server logs explicitly cleared.");

      // Broadcast the clear trigger to all active SSE listener instances
      sseClients.forEach(client => {
        try {
          client.res.write(`event: clear\ndata: {}\n\n`);
        } catch (e) {
