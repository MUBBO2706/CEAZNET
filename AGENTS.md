...
**Reference API Base URL:** `https://task-manager-ceaznet.vercel.app`
...
- **SQL Migration History:** `https://github.com/MUBBO2706/TASK-MANAGER` (Store schema/RPC/Edge Functions here to make migrations easy across apps)
- **Ceaznet Admin Panel:** `https://github.com/MUBBO2706/CEAZNET-ADMIN` (Admin Panel application built specifically to manage this current client application)
...
1. **Absolutely NO Workspace Pollution:** Whenever fetching external codebases via `degit` or Git (e.g., Ceaznet Admin or SQL Migration repo), **NEVER** clone them into the existing application's working directory (`.` or `./admin_repo` or any local folder). 
   - All clones **MUST** be placed in the temporary `/tmp` directory (e.g., `npx -y degit MUBBO2706/CEAZNET-ADMIN /tmp/admin_repo`).
...
3. **ROLE RESTRICTION: Read-Only Production & Write-Only Staging (CRITICAL):**
   - **Production (Read-Only):** You ONLY have read access to Production projects. You are STRICTLY FORBIDDEN from creating, modifying, or deleting tasks in a production project. Attempts to modify or delete Production tasks directly will be rejected with `403 Forbidden`.
   - **Staging (Write & Delete Access):** You can ONLY write (POST/PUT) or delete (DELETE) tasks in projects whose names end with `[STAGING]`.

   *Step A: Fetch Projects*
   Run this snippet to get the list of all projects:
   ```javascript
   npx -y tsx -e "fetch('https://task-manager-ceaznet.vercel.app/api/projects', { headers: { 'x-api-key': 'sk_sync_b4k92jdm10' } }).then(res=>res.json()).then(console.log).catch(console.error)"
   ```
   ...
   *Step B: Resolve Staging Replica*
   ...
   - **If Staging DOES NOT exist:** Create one by calling the endpoint with the *Production* `projectId`:
     ```javascript
     npx -y tsx -e "fetch('https://task-manager-ceaznet.vercel.app/api/ai/create-staging', { method: 'POST', headers: { 'Content-Type': 'application/json', 'x-api-key': 'sk_sync_b4k92jdm10' }, body: JSON.stringify({ projectId: '<PRODUCTION_PROJECT_ID>' }) }).then(res=>res.json()).then(console.log).catch(console.error)"
     ```
   - Use the `[STAGING]` project's ID for all subsequent actions.

   *Step C: Push, Update, or Delete ONLY in Staging*
   ...
   - To DELETE an existing task: Use `DELETE /api/ai/write/:taskId` with header `x-api-key`. The target `taskId` MUST belong to a `[STAGING]` project. If you mistakenly pass a Production task ID, deletion will be blocked with `403 Forbidden`.
   - **ABSOLUTE BAN:** Attempting to update, delete, or push to a `taskId` or `projectId` that belongs to production is a severe role violation.

4. **Target API Ecosystem Guide (Ceaznet Task Manager):**
   - **Base URL**: `https://task-manager-ceaznet.vercel.app`
   - **Auth Header**: `x-api-key: sk_sync_b4k92jdm10`

   **Official Standard Endpoints (`/api/ai/*`):**
   1. `GET /api/projects`: Fetch list of all active projects (including `[STAGING]` replicas).
   2. `GET /export.json?projectId=<PROJECT_ID>`: Export all tasks, SQL queries, metadata, and Edge Functions for a project.
   3. `POST /api/ai/create-staging`: Body `{ "projectId": "<PRODUCTION_PROJECT_ID>" }`. Clones production into `<Project Name> [STAGING]`.
   4. `POST /api/ai/write`: Body `{ "projectId": "<STAGING_PROJECT_ID>", "title": "...", "sql": "...", "description": "...", "type": "sql" | "edge_function" }`. Creates a task in Staging.
   5. `PUT /api/ai/write/:taskId`: Body `{ "title": "...", "sql": "...", "description": "...", "type": "sql" | "edge_function" }`. Updates an existing task in Staging.
   6. `DELETE /api/ai/write/:taskId`: Header `x-api-key`. Deletes an existing task from a `[STAGING]` project. (Production task IDs are blocked with 403 Forbidden).
   7. `POST /api/ai/merge-staging`: Body `{ "stagingProjectId": "<STAGING_PROJECT_ID>", "prodProjectId": "<PRODUCTION_PROJECT_ID>" }`. Merges validated staging changes into Production and cleans up the staging replica.

5. **Reporting Changes & Handling Confusion:**
...
### 🚀 Staging System Architecture & Developer Setup

This section outlines the feature implementation workflow to bypass manual copy-pasting while maintaining safety. The AI pushes updates to a **Staging (Testing)** project/environment for verification, which can then be merged to Production.

#### 📌 Concepts
The AI should not push directly into the "Production" project tasks to avoid breaking active services. Instead:
1. The AI pushes code to a **Staging** replica project.
2. The developer tests the implementation in Staging.
3. Once validated, the developer clicks **Merge** in the admin panel (or triggers `POST /api/ai/merge-staging`) to apply the changes to Production.

#### 🛠️ Developer Setup & Schema Reference
1. **Database Schema Updates**: Link Staging to Production in the database by adding environment tracking or reference columns:
   ```sql
...
2. **API Specification for AI Write Access**:
   - **READ Endpoint**: `GET /export.json?projectId=<STAGING_PROJECT_ID>` (returns tasks for the specific project).
   - **WRITE Endpoint**: `POST /api/ai/write` 
     - Body: `{ "projectId": "<STAGING_PROJECT_ID>", "title": "...", "sql": "...", "description": "...", "type": "sql" | "edge_function" }`
   - **MERGE Endpoint**: `POST /api/ai/merge-staging`
     - Body: `{ "stagingProjectId": "<STAGING_PROJECT_ID>" }`

3. **UI/UX Project Replication**:
   - Provide a **"Create Staging Replica"** option in the Admin Panel to duplicate a project and its tasks to `<Project Name> [STAGING]`.

4. **UI/UX Merge Option**:
   - In the Staging project view of the Admin Panel, add a **"Merge to Production"** button to compare tasks, view diffs, and upsert them safely into Production.
...
- **Vercel Serverless Architecture & Real-Time Caching:** This application is deployed on Vercel, which operates in a serverless, stateless environment (lambda functions spin down, and local filesystem/memory are wiped). Consequently, **DO NOT rely on in-memory server-side variables or local server files for persistent caching**. All cache operations for deviceCache, imageCache, and sessionCache **must always** be executed in real-time, pulling directly from Telegram and writing directly to Telegram on every load and save request to prevent stale, inconsistent state.

---

## 4. Edge Functions & Preserved Source Code

The source code for all edge functions used in this application is strictly preserved in the SQL Task Manager repository/database.

**Currently Used Edge Functions (List):**
1. `send-password-alert`
2. `update-article-stats`
3. `update-news`

**Directives for Edge Functions:**
