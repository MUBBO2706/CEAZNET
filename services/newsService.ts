...
export const fetchNews = async (category: string, summary: boolean = false): Promise<NewsArticle[]> => {
    ...
    const fetchPromise = (async () => {
        try {
            ...
            const response = await fetchApi(`/api/news?category=${encodeURIComponent(category)}${summary ? '&lite=true' : ''}`, {
                method: 'GET',
                headers: {
...
export const fetchArticleDetails = async (url: string): Promise<NewsArticle> => {
    ...
    const fetchPromise = (async () => {
        try {
            const response = await fetchApi(`/api/news?url=${encodeURIComponent(url)}`, {
                method: 'GET',
                headers: {
                    'Content-Type': 'application/json'
                }
