...
export const autoCategorizeByDescription = async (
    ...
    const categoryLookup = new Map<string, { label: string; id: string; iconName: string; type: string }>();

    const indexCategory = (c: { id: string; label?: string; iconName?: string; type?: string }) => {
        const label = c.label || c.id;
        const entry = { label, id: c.id, iconName: c.iconName || 'solar:tag-bold-duotone', type: (c.type || type) as string };
        const variants = [
            label.toLowerCase().trim(),
            c.id.toLowerCase().trim(),
            label.replace(/[_\s-]+/g, ' ').toLowerCase().trim(),
            c.id.replace(/[_\s-]+/g, ' ').toLowerCase().trim(),
            c.id.replace(/\s+/g, '_').toLowerCase().trim()
        ];
        variants.forEach(v => {
            if (v && !categoryLookup.has(v)) {
                categoryLookup.set(v, entry);
            }
        });
    };
    ...
    // Index all other standard categories and remaining custom categories as secondary fallback
    const otherTypes: Array<'expense' | 'income' | 'transfer'> = (['expense', 'income', 'transfer'] as const).filter(t => t !== type);
    otherTypes.forEach(t => {
        (CATEGORY_CONFIG[t] || []).forEach(c => indexCategory({ ...c, type: t }));
    });
    customCats.forEach(c => indexCategory(c));
...
Respond strictly in JSON:
{
  ...
  "reason": string
    ...
    const findInLookup = (query?: string) => {
        if (!query) return undefined;
        const q = query.toLowerCase().trim();
        return categoryLookup.get(q) 
            || categoryLookup.get(q.replace(/[_\s-]+/g, ' '))
            || categoryLookup.get(q.replace(/\s+/g, '_'));
    };

    // If successfully matched to an existing category, return immediately with local icon!
    const matchedCategoryKey = matchParsed.matchedCategory || (matchParsed.isExisting ? matchParsed.suggestedCategoryName : undefined);
    const existingMatch = findInLookup(matchedCategoryKey) || (matchParsed.suggestedCategoryName ? findInLookup(matchParsed.suggestedCategoryName) : undefined);

    if (existingMatch || (matchParsed.isExisting && matchParsed.matchedCategory)) {
        const catLabel = existingMatch ? existingMatch.label : (matchParsed.matchedCategory || matchParsed.suggestedCategoryName || 'General');
        const catId = existingMatch ? existingMatch.id : (matchParsed.matchedCategory || matchParsed.suggestedCategoryName || 'General');
        const icon = existingMatch?.iconName || 'solar:tag-bold-duotone';
