...
export async function fetchImageProxy(url: string): Promise<string | null> {
  ...
  const fetchPromise = (async () => {
    try {
      const response = await fetchApi(`/api/image-proxy?url=${encodeURIComponent(url)}`);
      if (response.ok) {
...
export async function batchFetchAndCacheImages(urls: string[]): Promise<void> {
  ...
  try {
    const response = await fetchApi('/api/image-proxy?action=batch_warmup', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
      },
      body: JSON.stringify({ urls: filteredUrls, warmOnly: true })
    });
    if (!response.ok) {
        ...
        if (res.status === 'failed') {
          ...
          if (typeof window !== 'undefined') {
            window.dispatchEvent(new CustomEvent('add-server-log', {
              detail: {
                type: 'info',
                message: `Image unresolvable: ${res.url}`,
