...
export const deleteConversation = async (id: string, user: User | null) => {
    ...
    if (audioUrlToDelete && audioUrlToDelete.startsWith('tg://')) {
        try {
            ...
            console.warn('Failed to delete audio from Telegram:', err);
        }
    }
...
export const getArticleConversation = async (articleUrl: string, user: User | null): Promise<ArticleConversation | undefined> => {
    ...
    const promise = (async () => {
        try {
            ...
            console.warn("Failed to fetch article conversation", err);
            return undefined;
        } finally {
...
export const getPublicArticleCache = async (articleUrl: string): Promise<{ title: string; content: string } | null> => {
    ...
    const promise = (async () => {
        try {
            ...
            console.warn("Failed to fetch public article cache", err);
            return null;
        } finally {
...
const saveLocalInteraction = (interaction: UserArticleInteraction) => {
    try {
        ...
        console.warn("Failed to save local interaction", e);
    }
};
...
export const incrementStat = async (articleUrl: string, stat: 'views' | 'likes' | 'bookmarks', increment: boolean) => {
    try {
        ...
        console.warn(`Failed to increment stat '${stat}' for article ${articleUrl}`, e);
    }
};
    ...
    try {
        ...
        if (error) {
            if (error.code !== 'PGRST116') {
                console.warn("Error fetching article stats", error);
            }
            return null;
        }
        ...
        console.warn("Failed to get article stats", e);
        return null;
    }
};
    ...
    if (user) {
        ...
        cachedNotesPromise = (async () => {
            try {
                ...
                const response = await fetchApi('/api/db/query?q=fetch_notes', {
                    method: 'POST',
                    credentials: 'omit',
                    headers: { 
    ...
    if (user) {
        ...
        const promise = (async () => {
            try {
                ...
                console.warn("Error in getNoteById:", err);
                return null;
            } finally {
                pendingSingleNotePromises.delete(id);
            }
...
export const getPublicSharedNote = async (id: string, exp?: string | null, sig?: string | null): Promise<Note | null> => {
    try {
        ...
        console.warn("Failed to fetch public shared note", error);
...
export const getCustomCategories = async (user: User | null): Promise<CustomCategoryItem[]> => {
    try {
        ...
        console.warn("Error fetching custom categories:", e);
        const local = await getFromLocalDB<CustomCategoryItem[]>(STORES.SETTINGS, 'custom_categories');
        return Array.isArray(local) ? local : [];
    }
};
    ...
    try {
        ...
        if (user) {
            try {
                ...
                if (error) {
                    console.warn("Error saving custom categories to Supabase user_metadata:", error);
                }
            } catch (err) {
...
export const getHiddenCategories = async (user: User | null): Promise<string[]> => {
    try {
        ...
        console.warn("Error fetching hidden categories:", e);
        return [];
    }
};
...
export const hideCategory = async (categoryId: string, user: User | null): Promise<string[]> => {
    try {
        ...
        console.warn("Error hiding category:", e);
        return [];
    }
};
...
export const unhideCategory = async (categoryId: string, user: User | null): Promise<string[]> => {
    try {
        ...
        console.warn("Error unhiding category:", e);
        return [];
    }
};
...
const categoryUsageImpactPending = new Map<string, Promise<CategoryUsageImpact>>();

export const clearCategoryUsageImpactCache = (categoryId?: string) => {
    if (categoryId) {
        const target = categoryId.toLowerCase().trim();
        for (const key of Array.from(categoryUsageImpactCache.keys())) {
            if (key.endsWith(`:${target}`)) {
                categoryUsageImpactCache.delete(key);
            }
        }
    } else {
...
export const getCategoryUsageImpact = async (
    categoryId: string, 
    user: User | null,
    profileId?: string | null
): Promise<CategoryUsageImpact> => {
    const targetLower = categoryId.toLowerCase().trim();
    const isDefault = profileId === null || profileId === 'default';
    const isAll = profileId === 'all' || profileId === undefined;
    const profileKey = isAll ? 'ALL' : (isDefault ? 'DEFAULT' : profileId);
    const cacheKey = `${user ? user.id : 'ANON'}:${profileKey}:${targetLower}`;
    ...
    if (categoryUsageImpactPending.has(cacheKey)) {
        return categoryUsageImpactPending.get(cacheKey)!;
    }

    const promise = (async (): Promise<CategoryUsageImpact> => {
        try {
            ...
            if (user) {
                let query = supabase
                    ...
                    .select('id, user_id, amount, type, category, description, payment_method, transaction_date, profile_id')
                    .eq('user_id', user.id)
                ...
                if (error) {
                    ...
                    affected = localTxs.filter(t => {
                        const matchCat = (t.category || '').toLowerCase().trim() === targetLower;
                        if (!matchCat) return false;
                        if (isAll) return true;
                        if (isDefault) return !t.profile_id || t.profile_id === 'default';
            ...
            console.warn("Error getting category usage impact:", e);
            return {
                categoryId,
                count: 0,
                totalAmount: 0,
            ...
            categoryUsageImpactPending.delete(cacheKey);
        }
    })();

    categoryUsageImpactPending.set(cacheKey, promise);
    return promise;
};
...
export const reassignCategoryTransactions = async (
    ...
    try {
        ...
        if (user) {
            let query = supabase
                .from('finance_transactions')
                .select('id, user_id, amount, type, category, description, payment_method, transaction_date, created_at, profile_id')
                .eq('user_id', user.id)
                .ilike('category', targetLower);
        ...
        if (user) {
            ...
            if (updateError) {
                logSupabaseError("Error updating category transactions", updateError);
                throw updateError;
            }
        }

        // Update local store and in-memory caches
        ...
        console.warn("Error reassigning category transactions:", e);
        throw e;
    }
};
...
export const getTransactionsPaginated = async (
    ...
    const promise = (async () => {
        try {
            ...
            console.warn("Unexpected error in getTransactionsPaginated:", err);
            return {
                data: [],
                totalCount: 0,
                page,
...
export const deleteDairyEntry = async (id: string, user: User | null) => {
    ...
    if (user) {
        try {
            ...
            console.warn("Error fetching entry to delete", e);
        }
    } else {
        const entries = await getDairyEntries(null);
...
export const importDairyData = async (jsonData: string, user: User | null): Promise<boolean> => {
    try {
        ...
        console.warn("Error importing dairy data:", error);
        return false;
    }
};
...
export const saveTranslationHistoryItem = async (
    ...
    if (user) {
        try {
            ...
            console.warn("Failed to insert translation history in Supabase:", e);
        }
    }
    ...
    try {
        ...
        console.warn("Local history cache error:", e);
    }
};
...
export const getTranslationHistory = async (
    ...
    if (user) {
        try {
            ...
            console.warn("Failed to fetch translation history from Supabase:", e);
        }
    }
...
export const clearTranslationHistory = async (user: User | null): Promise<void> => {
    if (user) {
        try {
            ...
            console.warn("Failed to clear remote translation history:", e);
        }
    }
    ...
    try {
        ...
        console.warn("Failed to clear local translation history cache:", e);
    }
};
...
export const deleteGalleryItem = async (id: string, user: User | null) => {
    ...
    if (urlToDelete && urlToDelete.startsWith('tg://')) {
        try {
            ...
            console.warn('Failed to delete file from Telegram:', err);
        }
    }
