...
const generateDateHeaderHtml = (dateStr: string): string => {
    ...
    return `<!-- DATE_HEADER_START --><h3 class="flex items-center gap-2 text-sm font-bold text-gray-800 dark:text-gray-200 mt-1.5 mb-1"><span class="f-icon fi-calendar text-gray-500"></span> ${dateStr}</h3><!-- DATE_HEADER_END -->`;
};
...
const generateFooterStats = (txLines: string[]): string => {
    ...
    // Structured HTML Blockquote for the Daily Total
    return `<blockquote class="my-0 py-0.5 px-2 border-l-2 border-indigo-400 dark:border-indigo-600 rounded-r-xl flex items-center justify-between gap-1.5" style="margin-top:0!important;margin-bottom:0!important;"><span class="not-italic flex items-center justify-between w-full"><span class="text-[10px] text-gray-400 dark:text-gray-500 font-bold uppercase tracking-wider">Daily Total</span><span class="flex items-center gap-2">${incomePart}<span class="text-gray-300 dark:text-gray-600 text-[10px]">|</span>${expensePart}<span class="text-gray-300 dark:text-gray-600 text-[10px]">|</span>${balancePart}</span></span></blockquote> <!-- daily_stats -->`;
};
...
const generateDailyVerdict = (txs: Transaction[]): string => {
    ...
    return `<!-- daily_verdict_start --><div class="mt-0 p-1.5 rounded-xl border flex items-center justify-between ${verdictBg}" style="margin-top:10px!important;"><div class="flex items-center gap-2"><span class="f-icon ${iconClass} ${verdictColor}"></span><span class="text-xs font-bold ${verdictColor}">${verdictTitle}</span></div>${topCatHtml}</div><!-- daily_verdict_end -->`;
};

/**
 * Generates a Dashboard Snapshot header for the top of the note as a Markdown Table.
 */
const generateSnapshotHeader = (transactions: Transaction[], profileName: string, existingContent?: string) => {
    let income = 0;
    let expense = 0;
    let highestTx = 0;
    const catTotals: Record<string, number> = {};
    ...
    // Extract first synced from existing content if it exists
    let firstSyncedTime = dateStr;
    if (existingContent) {
        const firstSyncedMatch = existingContent.match(/First Synced:\s*([^\n<|]+)/i);
        if (firstSyncedMatch) {
            firstSyncedTime = firstSyncedMatch[1].trim();
        }
    }
    ...
    // 1. Expense Breakdown Card (Container-less)
    const breakdownColors = ['bg-rose-500', 'bg-orange-500', 'bg-amber-500', 'bg-emerald-500', 'bg-blue-500'];
    let breakdownContent = "";
    
    if (sortedCats.length === 0) {
        breakdownContent = `<p class="text-xs text-gray-400 italic text-center py-1">No expenses recorded yet</p>`;
    } else {
    ...
    // Container-less Expense Breakdown (Only Title and Content)
    const expenseBreakdownCard = `<!-- FINANCE_WIDGET_START --><div class="mb-2">` +
        `<div class="flex items-center gap-2 mb-2"><span class="f-icon fi-expense text-rose-500"></span><span class="text-[10px] font-bold uppercase tracking-widest text-gray-500 dark:text-gray-400">Expense Breakdown</span></div>` +
        breakdownContent +
    `</div><!-- FINANCE_WIDGET_END -->`;

    // 2. 4-Grid Stats Cards HTML with Right Side Contextual Metrics & Thin Accent Borders
    const expenseRatio = income > 0 ? ((expense / income) * 100).toFixed(1) : '0';
    const statsCards = `<!-- FINANCE_WIDGET_START --><div class="grid grid-cols-2 md:grid-cols-4 gap-2 items-stretch mb-2">` +
        `<div class="p-2.5 bg-emerald-50/80 dark:bg-emerald-950/20 rounded-2xl border border-emerald-300 dark:border-emerald-800/60 shadow-none flex flex-col justify-between">` +
            `<div class="flex items-center gap-1.5 mb-1 opacity-80"><span class="f-icon fi-income text-emerald-500"></span><span class="text-[10px] font-bold uppercase tracking-wider text-emerald-800 dark:text-emerald-300">Income</span></div>` +
            `<div class="flex items-baseline justify-between gap-1">` +
                `<div class="text-base md:text-lg font-mono font-bold text-emerald-700 dark:text-emerald-400">₹${income.toLocaleString('en-IN')}</div>` +
                `<div class="text-[10px] font-medium text-emerald-600/80 dark:text-emerald-400/80 text-right">Inflow</div>` +
            `</div>` +
        `</div>` +
        `<div class="p-2.5 bg-rose-50/80 dark:bg-rose-950/20 rounded-2xl border border-rose-300 dark:border-rose-800/60 shadow-none flex flex-col justify-between">` +
            `<div class="flex items-center gap-1.5 mb-1 opacity-80"><span class="f-icon fi-expense text-rose-500"></span><span class="text-[10px] font-bold uppercase tracking-wider text-rose-800 dark:text-rose-300">Expense</span></div>` +
            `<div class="flex items-baseline justify-between gap-1">` +
                `<div class="text-base md:text-lg font-mono font-bold text-rose-700 dark:text-rose-400">₹${expense.toLocaleString('en-IN')}</div>` +
                `<div class="text-[10px] font-medium text-rose-600/80 dark:text-rose-400/80 text-right">${expenseRatio}% spent</div>` +
            `</div>` +
        `</div>` +
        `<div class="p-2.5 bg-gray-50 dark:bg-gray-800/80 rounded-2xl border border-gray-300 dark:border-gray-700 shadow-none flex flex-col justify-between">` +
            `<div class="flex items-center gap-1.5 mb-1 opacity-80"><span class="f-icon fi-wallet text-gray-500"></span><span class="text-[10px] font-bold uppercase tracking-wider text-gray-600 dark:text-gray-400">Net Flow</span></div>` +
            `<div class="flex items-baseline justify-between gap-1">` +
                `<div class="text-base md:text-lg font-mono font-bold text-gray-800 dark:text-gray-200">${sign}₹${Math.abs(balance).toLocaleString('en-IN')}</div>` +
                `<div class="text-[10px] font-medium text-gray-600/80 dark:text-gray-400/80 text-right">${savingsRate >= 0 ? '+' : ''}${savingsRate.toFixed(1)}% net</div>` +
            `</div>` +
        `</div>` +
        `<div class="p-2.5 bg-blue-50/80 dark:bg-blue-950/20 rounded-2xl border border-blue-300 dark:border-blue-800/60 shadow-none flex flex-col justify-between">` +
            `<div class="flex items-center gap-1.5 mb-1 opacity-80"><span class="f-icon fi-alert text-blue-500"></span><span class="text-[10px] font-bold uppercase tracking-wider text-blue-700 dark:text-blue-300">Saved</span></div>` +
            `<div class="flex items-baseline justify-between gap-1">` +
                `<div class="text-base md:text-lg font-mono font-bold text-blue-700 dark:text-blue-400">${savingsRate.toFixed(1)}%</div>` +
                `<div class="text-[10px] font-medium text-blue-600/80 dark:text-blue-400/80 text-right">₹${Math.max(0, balance).toLocaleString('en-IN')} rem.</div>` +
            `</div>` +
        `</div>` +
    `</div><!-- FINANCE_WIDGET_END -->`;

    // 3. 4-Grid Insights HTML (Compact Layout with Right-Side Enriched Secondary Metrics)
    const incCount = transactions.filter(t => t.type === 'income').length;
    const expCount = transactions.filter(t => t.type === 'expense').length;
    const highestTxObj = [...transactions].sort((a, b) => Number(b.amount) - Number(a.amount))[0];
    const highestCat = highestTxObj ? highestTxObj.category : 'N/A';
    const txAvgVsDaily = dailyAvg > 0 ? (avgTx / dailyAvg).toFixed(1) : '1.0';

    const insightsGrid = `<!-- FINANCE_WIDGET_START --><div class="grid grid-cols-2 md:grid-cols-4 gap-2 mt-0 items-stretch mb-2">` +
        `<div class="p-2.5 bg-indigo-50/80 dark:bg-indigo-950/20 rounded-2xl border border-indigo-200 dark:border-indigo-800/50 flex flex-col justify-between">` +
            `<div class="flex items-center gap-1.5 mb-1 opacity-80"><span class="f-icon fi-transfer text-indigo-500"></span><span class="text-[10px] font-bold uppercase tracking-wider text-indigo-800 dark:text-indigo-300">Tx Volume</span></div>` +
            `<div class="flex items-baseline justify-between gap-1">` +
                `<div class="text-base font-mono font-bold text-indigo-900 dark:text-indigo-200">${transactions.length}</div>` +
                `<div class="text-[10px] font-medium text-indigo-600/80 dark:text-indigo-300/80 text-right">${incCount} In · ${expCount} Out</div>` +
            `</div>` +
        `</div>` +
        `<div class="p-2.5 bg-amber-50/80 dark:bg-amber-950/20 rounded-2xl border border-amber-200 dark:border-amber-800/50 flex flex-col justify-between">` +
            `<div class="flex items-center gap-1.5 mb-1 opacity-80"><span class="f-icon fi-calendar text-amber-500"></span><span class="text-[10px] font-bold uppercase tracking-wider text-amber-800 dark:text-amber-300">Daily Avg</span></div>` +
            `<div class="flex items-baseline justify-between gap-1">` +
                `<div class="text-base font-mono font-bold text-amber-900 dark:text-amber-200">₹${dailyAvg.toFixed(0)}</div>` +
                `<div class="text-[10px] font-medium text-amber-600/80 dark:text-amber-300/80 text-right">₹${(dailyAvg * 30).toLocaleString('en-IN', {maximumFractionDigits: 0})}/mo</div>` +
            `</div>` +
        `</div>` +
        `<div class="p-2.5 bg-rose-50/80 dark:bg-rose-950/20 rounded-2xl border border-rose-200 dark:border-rose-800/50 flex flex-col justify-between">` +
            `<div class="flex items-center gap-1.5 mb-1 opacity-80"><span class="f-icon fi-expense text-rose-500"></span><span class="text-[10px] font-bold uppercase tracking-wider text-rose-800 dark:text-rose-300">Highest</span></div>` +
            `<div class="flex items-baseline justify-between gap-1">` +
                `<div class="text-base font-mono font-bold text-rose-900 dark:text-rose-200">₹${highestTx.toLocaleString('en-IN')}</div>` +
                `<div class="text-[10px] font-medium text-rose-600/80 dark:text-rose-300/80 text-right truncate max-w-[80px]">${highestCat}</div>` +
            `</div>` +
        `</div>` +
        `<div class="p-2.5 bg-teal-50/80 dark:bg-teal-950/20 rounded-2xl border border-teal-200 dark:border-teal-800/50 flex flex-col justify-between">` +
            `<div class="flex items-center gap-1.5 mb-1 opacity-80"><span class="f-icon fi-alert text-teal-500"></span><span class="text-[10px] font-bold uppercase tracking-wider text-teal-800 dark:text-teal-300">Tx Avg</span></div>` +
            `<div class="flex items-baseline justify-between gap-1">` +
                `<div class="text-base font-mono font-bold text-teal-900 dark:text-teal-200">₹${avgTx.toFixed(0)}</div>` +
                `<div class="text-[10px] font-medium text-teal-600/80 dark:text-teal-300/80 text-right">${txAvgVsDaily}x of daily</div>` +
            `</div>` +
        `</div>` +
    `</div><!-- FINANCE_WIDGET_END -->`;

    // Construct the string with minimal newlines and tight separator placement, with subtle dividers
    return `<!-- REPORT_HEADER_START --><div class="flex items-baseline justify-between w-full mb-2"><h1 class="text-xl font-bold text-gray-900 dark:text-white flex items-center gap-2"><span class="f-icon fi-wallet text-indigo-500"></span> ${profileName}</h1><span class="text-xs text-gray-400 dark:text-gray-500 font-medium select-none">${transactions.length} Transactions</span></div><hr class="my-1.5 border-0 border-t-[0.5px] border-gray-200/50 dark:border-white/10" /><!-- REPORT_HEADER_END -->` +
        statsCards +
        `<hr class="my-1.5 border-0 border-t-[0.5px] border-gray-200/50 dark:border-white/10" />` +
        expenseBreakdownCard + 
        `<hr class="my-1.5 border-0 border-t-[0.5px] border-gray-200/50 dark:border-white/10" />` +
        insightsGrid +
        `<hr class="my-1.5 border-0 border-t-[0.5px] border-gray-200/50 dark:border-white/10" />` +
        `<!-- REPORT_FOOTER_START --><div class="flex flex-row justify-between items-center gap-2 mt-1.5 mb-1.5 w-full"><span class="text-[10px] text-gray-400 leading-none">First Synced: ${firstSyncedTime}</span><span class="text-[10px] text-gray-400 leading-none text-right">Last Synced: ${dateStr}</span></div><hr class="my-0 border-0 border-t-[0.5px] border-gray-200/50 dark:border-white/10" /><!-- REPORT_FOOTER_END -->`;
};
 ...
 */
const rebuildNoteContent = (transactions: Transaction[], profileName: string, existingContent?: string): string => {
    ...
    // 2. Build New Content
    let newContent = generateSnapshotHeader(transactions, profileName, existingContent); 
    ...
    sortedHeaders.forEach((headerKey, index) => {
        ...
        // Add HTML Header for this day without extra newlines (newline only to separate from preceding days)
        if (index > 0) {
            newContent += `\n`;
        }
        newContent += `${generateDateHeaderHtml(headerKey)}\n`;
 ...
 */
export const getProfileName = async (user: User | null, profileId: string | null | undefined): Promise<string> => {
