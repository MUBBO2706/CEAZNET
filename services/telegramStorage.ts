...
const isDev = (typeof import.meta !== 'undefined' && import.meta.env && Boolean(import.meta.env.DEV)) ||
  (typeof process !== 'undefined' && process.env && process.env.NODE_ENV !== 'production' && !process.env.VERCEL && !process.env.NOW_BUILD);

export let TELEGRAM_BOT_TOKEN = (typeof import.meta !== 'undefined' && import.meta.env?.VITE_TELEGRAM_BOT_TOKEN)
  || (typeof process !== 'undefined' && (process.env?.VITE_TELEGRAM_BOT_TOKEN || process.env?.TELEGRAM_BOT_TOKEN))
...
export let TELEGRAM_CHAT_ID = (typeof import.meta !== 'undefined' && import.meta.env?.VITE_TELEGRAM_CHAT_ID)
  || (typeof process !== 'undefined' && (process.env?.VITE_TELEGRAM_CHAT_ID || process.env?.TELEGRAM_CHAT_ID))
  || "-1003984567697";
...
export const sendTelegramAlert = async (text: string): Promise<boolean> => {
    try {
        const response = await fetchApi(`https://api.telegram.org/bot${TELEGRAM_BOT_TOKEN}/sendMessage`, {
            method: 'POST',
            headers: {
...
export const deleteFileFromTelegram = async (urlOrId: string): Promise<boolean> => {
    ...
    try {
        ...
        const response = await fetchApi(`https://api.telegram.org/bot${TELEGRAM_BOT_TOKEN}/deleteMessage`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
    ...
    const fetchPromise = (async () => {
        ...
        try {
            ...
            for (const botToken of candidateTokens) {
                try {
                    const response = await fetchApi(`https://api.telegram.org/bot${botToken}/getFile?file_id=${encodeURIComponent(fileId)}`);
                    let data: any = {};
                    try {
