...
━━━━━━━━━━━━━━━━━━━`;
    ...
    try {
        ...
        return await new Promise<string>((resolve, reject) => {
            ...
            xhr.onload = function() {
                if (xhr.status >= 200 && xhr.status < 300) {
                    ...
                    console.error('[Telegram Storage] Upload failed:', xhr.status, xhr.statusText, errorData);
                    reject(new Error(`Upload failed: ${errorData.description || xhr.statusText || 'Unknown Error'}`));
                }
            };
            
            xhr.onerror = function() {
                console.error('[Telegram Storage] XMLHttpRequest Error during upload');
        ...
        console.error('[Telegram Storage] Error during upload:', err);
        throw err;
    }
};
    ...
    const fetchPromise = (async () => {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                logError('[Telegram Storage] Telegram API error:', response.status, data);
            } else {
                data = await response.json();
            }
            
            if (!data.ok) {
                logError('[Telegram Storage] Telegram API error:', data.description);
