...
const App: React.FC = () => {
  ...
  const urlExploreCategory = useMemo(() => {
    ...
    if (parts.length === 3 && parts[1] === "explore" && allExploreCategories.includes(parts[2])) {
      return parts[2];
    }
    return null;
  }, [currentPath]);
  ...
  useEffect(() => {
    ...
    const sendHeartbeat = async (statusOverride?: string) => {
        try {
          ...
          const { getPersistentDeviceId, getExactDeviceName } = await import('./utils/deviceUtils');
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (data && data.length > 0 && !data[0].id.includes("fallback")) {
            const terminatedSession = data.find(
        ...
        <div className="flex flex-1 overflow-hidden relative">
          ...
          <div id="main-content-area" className="flex-1 flex flex-col overflow-hidden relative">
            ...
            <div
              ...
              className={`flex-1 min-h-0 scrollbar-hide ${currentView === "not-found" ? "flex flex-col overflow-hidden" : ["live-conversation", "reading", "explore", "article-reader", "settings", "finance", "notes", "shared-note", "about", "features", "privacy-policy", "terms-of-service", "molecule-viewer", "gallery"].includes(currentView) ? "overflow-hidden" : "overflow-y-auto"}`}
            >
              <ViewRenderer
                currentView={currentView}
