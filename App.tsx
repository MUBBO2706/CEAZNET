...
const App: React.FC = () => {
  ...
  const [isTranslatorStatsOpen, setIsTranslatorStatsOpen] = useState(false);
  const [translatorUsage, setTranslatorUsage] = useState<{
    input: number;
    output: number;
  }>({ input: 0, output: 0 });
  const translatorUsageRef = useRef(translatorUsage);
  useEffect(() => {
    translatorUsageRef.current = translatorUsage;
  }, [translatorUsage]);
  ...
  useEffect(() => {
    ...
    const trackSession = async () => {
      try {
        // Prevent 400 Bad Request by cleaning up bloated/stale cookies if cookie jar exceeds 3KB
        try {
          if (typeof document !== 'undefined' && document.cookie && document.cookie.length > 3000) {
            document.cookie.split(';').forEach((c) => {
              const name = c.split('=')[0].trim();
              if (name && !name.startsWith('sb-') && !name.includes('csrf')) {
        ...
        const response = await fetchApi("/api/sessions?action=track", {
          method: "POST",
          credentials: "omit",
          headers: {
  ...
  // Track Remote Terminations of Active Session via instant Supabase Realtime
  useEffect(() => {
    if (!user || !session) return;
  ...
  const handleTranslationComplete = useCallback(
    (t: { 
      ...
      const currentUsage = translatorUsageRef.current;
      const newUsage = {
        input: currentUsage.input + t.input,
        output: currentUsage.output + t.output,
      };
      
      translatorUsageRef.current = newUsage;
      setTranslatorUsage(newUsage);
      ...
      saveTranslationHistoryItem({
        ...
        output_text: t.outputText || '',
        input_tokens: t.input,
        output_tokens: t.output,
        source_lang: t.source || 'Auto Detect',
        target_lang: t.target || 'English',
        model: t.model || 'Gemini 3.8 Flash',
      }, user).catch((e) =>
        console.error("Failed to save translation history item", e),
      );
    },
    [user],
  );
  ...
  return (
    <>
      ...
      <BroadcastPopup />
      <VersionUpdateModal />
      <TranslatorStatsModal 
        isOpen={isTranslatorStatsOpen}
        onClose={() => setIsTranslatorStatsOpen(false)}
        usage={translatorUsage}
        onReset={() => {
          translatorUsageRef.current = { input: 0, output: 0 };
          setTranslatorUsage({ input: 0, output: 0 });
        }}
        user={user}
      />
      <ChargingOverlay
