...
const isDev = (getEnvValue('NODE_ENV') !== 'production' && !getEnvValue('VERCEL') && !getEnvValue('NOW_BUILD'));

const supabaseUrl = getEnvValue('VITE_SUPABASE_URL') || getEnvValue('SUPABASE_URL') || 'https://itjurgqbvsqniphuehiz.supabase.co';
const supabaseKey = getEnvValue('SUPABASE_SERVICE_ROLE_KEY') || getEnvValue('SUPABASE_KEY') || getEnvValue('VITE_SUPABASE_ANON_KEY') || 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Iml0anVyZ3FidnNxbmlwaHVlaGl6Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzUyODM5NTgsImV4cCI6MjA5MDg1OTk1OH0.WSyZbgJ7rcbaTGCwURHTxQCHU9__F_ql75L6upVsVag';
...
export async function getTelegramCredentials(): Promise<{ token: string; cacheChatId: string; isConfigured: boolean }> {
  ...
  const defaultToken = "8403959177:AAFJrkcRCeTHTyS5uVBwlLKTE79dwq_HYzU";
  const defaultChatId = "-1003984567697";
