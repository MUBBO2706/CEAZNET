...
const TranslatorView: React.FC<TranslatorViewProps> = ({ onBack, onTranslationComplete }) => {
    ...
    return (
        <main className="relative z-10 h-full flex flex-col p-4 md:p-6 overflow-hidden pt-20 md:pt-24 max-w-[1600px] mx-auto w-full">
            <div className="flex-1 flex flex-col md:flex-row gap-6 md:gap-8 min-h-0">
                ...
                <div className="flex-1 flex flex-col min-h-[250px] md:min-h-0">
                    <div className="pb-3 border-b border-gray-200 dark:border-white/10">
                        <select value={sourceLang} onChange={(e) => setSourceLang(e.target.value)} className="w-full bg-transparent text-neutral-800 dark:text-gray-100 font-bold text-base focus:outline-none appearance-none cursor-pointer">
                            {sourceLanguages.map(lang => <option key={lang.code} value={lang.code} className="bg-white dark:bg-gray-900">{lang.name}</option>)}
                        </select>
                    </div>
                    <div className="flex-1 py-4 relative overflow-y-auto">
                         <textarea
                            ref={textareaRef}
                            value={inputText}
                            onChange={(e) => setInputText(e.target.value)}
                            placeholder="Enter text..."
                            className="w-full h-full bg-transparent resize-none focus:outline-none text-neutral-800 dark:text-gray-200 text-lg leading-relaxed placeholder:text-neutral-400 dark:placeholder:text-gray-500"
                        />
                    </div>
                    <div className="pt-3 border-t border-gray-200 dark:border-white/10 flex justify-between items-center text-xs text-neutral-500 dark:text-gray-400 font-mono">
                        ...
                        <div className="flex items-center gap-1">
                            <button onClick={handlePaste} className="p-2 rounded-full hover:bg-gray-100 dark:hover:bg-white/10 transition-colors" aria-label="Paste text">
                                <ClipboardPaste className="h-5 w-5" />
                            </button>
                            {inputText && (
                                <button onClick={() => setInputText('')} className="p-2 rounded-full hover:bg-gray-100 dark:hover:bg-white/10 transition-colors" aria-label="Clear input text">
                                    <X className="h-5 w-5" />
                                </button>
                            )}
                        </div>
                ...
                <div className="flex-1 flex flex-col min-h-[250px] md:min-h-0">
                     <div className="pb-3 border-b border-gray-200 dark:border-white/10">
                        <select value={targetLang} onChange={(e) => setTargetLang(e.target.value)} className="w-full bg-transparent text-neutral-800 dark:text-gray-100 font-bold text-base focus:outline-none appearance-none cursor-pointer">
                            {targetLanguages.map(lang => <option key={lang.code} value={lang.code} className="bg-white dark:bg-gray-900">{lang.name}</option>)}
                        </select>
                    </div>
                    <div className="flex-1 py-4 relative overflow-y-auto">
                        {isLoading ? (
                            ...
                            <div className="h-full flex flex-col items-center justify-center text-neutral-400 dark:text-gray-500">
                    ...
                    <div className="pt-3 border-t border-gray-200 dark:border-white/10 flex justify-between items-center text-xs text-neutral-500 dark:text-gray-400 font-mono">
                        ...
                        {!isLoading && outputText && (
                           <button onClick={handleCopy} className="p-2 rounded-full hover:bg-gray-100 dark:hover:bg-white/10 transition-colors" aria-label="Copy translation">
