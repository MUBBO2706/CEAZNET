...
export const SessionDetailsView: React.FC<SessionDetailsViewProps> = ({
  ...
  const activeCount = useMemo(() => {
    if (sessionStats && typeof sessionStats.active === 'number') {
      return sessionStats.active;
    }
    return sessions.filter(s => getSessionStatus(s).status === 'active').length;
  }, [sessions, currentSessionKey, sessionStats]);
  ...
  const deletableInactiveSessionsCount = useMemo(() => {
    if (sessionStats && typeof sessionStats.logged_out === 'number') {
      return (sessionStats.logged_out + sessionStats.terminated + sessionStats.expired);
    }
    return sessions.filter(s => getSessionStatus(s).status !== 'active' && !s.is_current && s.session_key !== currentSessionKey).length;
  }, [sessions, currentSessionKey, sessionStats]);

  const getStatusCount = (opt: StatusFilter): number | null => {
    if (!sessionStats) return null;
    if (opt === 'all') return sessionStats.total;
    if (opt === 'active') return sessionStats.active;
    if (opt === 'logged_out') return sessionStats.logged_out;
    if (opt === 'terminated') return sessionStats.terminated;
    if (opt === 'expired') return sessionStats.expired;
    return null;
  };
              ...
              <div className="relative flex-1 md:flex-none min-w-0">
                ...
                >
                  <span className="truncate">
                    ...
                    {getStatusCount(statusFilter) !== null ? ` (${getStatusCount(statusFilter)})` : ''}
                  </span>
                ...
                {isStatusDropdownOpen && (
                  <>
                    ...
                    <div className="absolute left-0 mt-1.5 w-52 bg-[var(--profile-modal-bg)] border border-[var(--profile-card-border)] rounded-xl shadow-lg z-30 py-1 overflow-hidden animate-in fade-in slide-in-from-top-1">
