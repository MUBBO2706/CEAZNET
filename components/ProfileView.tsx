...
export const ProfileView: React.FC<ProfileViewProps> = ({
  ...
  const [currentSessionFilter, setCurrentSessionFilter] = useState<string>('all');
  ...
  const handleTerminateSession = async (sessionId: string) => {
    ...
    try {
      ...
      const res = await fetchApi('/api/sessions?action=terminate', {
        method: 'POST',
        credentials: 'omit',
        headers: {
  ...
  const handleDeleteSession = async (sessionId: string) => {
    if (!user || !session?.access_token) return;
    try {
      const res = await fetchApi('/api/sessions?action=delete', {
        method: 'POST',
        credentials: 'omit',
        headers: {
          'Content-Type': 'application/json',
  ...
  const handleDeleteAllInactive = async () => {
    ...
    try {
      ...
      for (const s of inactiveSessions) {
        await fetchApi('/api/sessions?action=delete', {
          method: 'POST',
          credentials: 'omit',
          headers: {
            'Content-Type': 'application/json',
  ...
  const getDuration = (startStr: string, endStr?: string, isOngoing: boolean = false) => {
    ...
    // Tier 1: Less than 24 hours (86,400 seconds)
    ...
    // Tier 2: 24 hours to < 7 days (86,400 to 604,800 seconds)
    ...
    // Tier 3: 7 days to < 30 days (604,800 to 2,592,000 seconds)
    if (totalSeconds < 2592000) {
      ...
      const parts = [`${weeks}w`];
      if (days > 0 || hours > 0) parts.push(`${days}d`);
      if (hours > 0) parts.push(`${hours}h`);
      return parts.join(' ');
    }

    // Tier 4: 30 days to < 365 days (2,592,000 to 31,536,000 seconds)
    if (totalSeconds < 31536000) {
      ...
      if (weeks > 0 || days > 0) parts.push(`${weeks}w`);
    ...
    const parts = [`${years}y`];
    if (months > 0 || weeks > 0) parts.push(`${months}mo`);
              ...
              <div className="space-y-2.5 border-t border-gray-200 dark:border-white/10 pt-4 text-left">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-neutral-500 font-mono text-[9px]">
                    Showing {Math.min(10, sessions.length)} of {sessionStats.total || sessions.length}
                  </span>
                </div>
            ...
            <div className="p-5 flex flex-col divide-y divide-[var(--profile-card-border)]/50 text-xs">
              ...
              <div className="py-2.5 flex flex-col sm:flex-row sm:items-center justify-between gap-1.5">
                ...
                <div className="flex flex-col sm:items-end gap-0.5">
                  ...
                  {(detailModalSession.is_current || detailModalSession.session_key === currentSessionKey) && (
              ...
              <div className="py-2.5 flex items-center justify-between gap-2">
                ...
                <span className="font-mono text-xs font-semibold text-[var(--profile-text-primary)]">
                  {detailModalSession.battery_percentage !== undefined && detailModalSession.battery_percentage !== null ? `${detailModalSession.battery_percentage}%` : 'N/A'}
                </span>
              </div>
              ...
              <div className="py-2.5 flex items-center justify-between gap-2">
                ...
                <span className="text-xs font-semibold text-[var(--profile-text-primary)]">
                  {getSessionActions(detailModalSession, userProfile.full_name || '', user?.email).actionBy}
                </span>
              </div>
              ...
              <div className="py-2.5 flex items-center justify-between gap-2">
                ...
                <span className="text-xs font-semibold text-[var(--profile-text-primary)]">
                  {getSessionActions(detailModalSession, userProfile.full_name || '', user?.email).actionFrom}
                </span>
              </div>
              ...
              <div className="py-2.5 flex flex-col gap-1">
                ...
                <span className="font-mono text-[10px] text-[var(--profile-text-muted)] break-all select-all">
                  {detailModalSession.id || detailModalSession.session_key}
                </span>
              </div>
            </div>

            <div className="p-4 border-t border-[var(--profile-card-border)] bg-[var(--profile-card-subtle-bg)] flex justify-end">
              <button
                onClick={() => setDetailModalSession(null)}
                className="px-4 py-2 rounded-xl bg-[var(--profile-accent)] text-white hover:bg-[var(--profile-accent)]/90 text-xs font-semibold transition-colors shadow-sm"
              >
                Close
              </button>
            </div>
