...
const NotesView: React.FC<NotesViewProps> = ({ user, onBack, searchQuery, setSearchQuery, setNotesHeaderState, isSuspended }) => {
    ...
    const handleSyncWallet = async () => {
        ...
        setIsSyncing(true);
        try {
            const pName = await getProfileName(user, profileId);
            const success = await syncAllTransactionsToNote(user, profileId, pName);
            if (success) {
...
`;
    ...
    useEffect(() => {
        if (selectedNote && editorRef.current) {
            if (!isNoteLoading && (isReadOnly || editorRef.current.innerHTML === '')) {
                editorRef.current.innerHTML = markdownToHtml(selectedNote.content || '');
            }
        }
    }, [selectedNote?.id, selectedNote?.content, isReadOnly, isNoteLoading]);
