...
import { 
    ...
    Image, RotateCw, Cpu, Loader2, MoreHorizontal, MoreVertical, 
...
export const DevTools = () => {
    ...
    const [activeTab, setActiveTab] = useState<'console' | 'network' | 'cache' | 'image-cache' | 'session-cache'>(() => {
        try {
            const saved = localStorage.getItem('devToolsActiveTab') ?? localStorage.getItem('devConsoleActiveTab');
            if (saved && ['console', 'network', 'cache', 'image-cache', 'session-cache'].includes(saved)) {
                return saved as 'console' | 'network' | 'cache' | 'image-cache' | 'session-cache';
            }
        } catch {}
        return 'console';
    });
    ...
    useEffect(() => {
        try {
            ...
            window.dispatchEvent(new CustomEvent('devToolsStateChange', { detail: { isOpen } }));
        } catch {}
    }, [isOpen]);
    ...
    useEffect(() => {
        const handleLoading = (e: any) => {
            if (e.detail && typeof e.detail.loading === 'boolean') {
                setIsSessionCacheLoading(e.detail.loading);
            }
        };
        window.addEventListener('session-cache-loading', handleLoading);
        return () => window.removeEventListener('session-cache-loading', handleLoading);
    }, []);
    ...
    return (
        ...
        >
            {!isOpen ? (
                ...
                <>
                    {/* Top Action Tab Bar */}
                    <div className="flex-none flex justify-between bg-[var(--dev-console-tab-bg)] border-t border-[var(--dev-console-border)] select-none z-10 w-full pointer-events-auto px-4 py-1.5 gap-2 text-[var(--dev-console-text-muted)] text-xs">
                        ...
                        <div className="flex items-center gap-2">
                            {activeTab === 'session-cache' ? (
                                <button 
                                    ...
                                    title="Sync Sessions"
                                >
                                    {isSessionCacheLoading ? <Loader2 size={13} className="animate-spin" /> : <RotateCw size={13} />}
                                </button>
                            ) : (
                            ...
                            {activeTab === 'image-cache' && (
                                <>
                                    ...
                                    >
                                        {isImageCacheLoading ? <Loader2 size={13} className="animate-spin" /> : <RotateCw size={13} />}
                                    </button>
                                </>
                            )}
                            <button 
