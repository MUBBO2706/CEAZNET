...
export const DevTools = () => {
    ...
    const [isMaximized, setIsMaximized] = useState(() => {
        try {
            const saved = localStorage.getItem('devToolsIsMaximized') ?? localStorage.getItem('devConsoleIsMaximized');
            return saved !== null ? JSON.parse(saved) : false;
        } catch {
            return false;
        }
    ...
    useEffect(() => {
        try {
            localStorage.setItem('devToolsIsMaximized', JSON.stringify(isMaximized));
        } catch {}
    }, [isMaximized]);
