...
// Global module-level flags and data caches to prevent data loss on unmount/remount
let globalCacheData: Record<string, string | null> = {};
...
export const DevicesTab: React.FC<DevicesTabProps> = ({ 
    ...
    const [cacheData, setCacheData] = useState<Record<string, string | null>>(() => globalCacheData);
    ...
    // Audit Logs states
    const [auditLogs, setAuditLogs] = useState<any[]>(() => globalAuditLogs);
    ...
    const [apiKeys, setApiKeys] = useState<any[]>(() => globalApiKeys);
...
export async function resolveDeviceModel(modelCode: string) {
  const response = await fetchApi('${origin}/api/device-mapper', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
...
*Generated by Ceaznet DevTools Device Mapper Console*`;
    ...
    const fetchApiKeys = async (force = false) => {
        ...
        try {
            const res = await fetchApi('/api/device-mapper?action=api_keys_list');
            if (res.ok) {
                const data = await res.json();
                if (data.keys) {
                    globalApiKeys = data.keys;
    ...
    const handleCreateApiKey = async (e?: React.FormEvent) => {
        ...
        try {
            const res = await fetchApi('/api/device-mapper?action=api_key_create', {
                method: 'POST',
    ...
    const handleDeleteApiKey = async (id: string) => {
        ...
        try {
            const res = await fetchApi('/api/device-mapper?action=api_key_delete', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
    ...
    const handleAiModelChange = async (model: string) => {
        ...
        try {
            await fetchApi('/api/device-mapper?action=set_ai_model', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
    ...
    useEffect(() => {
        if (!isDeviceCacheLoaded && !globalIsDeviceCacheFetching) {
            fetchCacheData();
        }
    }, [isDeviceCacheLoaded]);

    const fetchCacheData = async (force = false) => {
        if (globalIsDeviceCacheFetching || globalIsAiModelFetching) return;
        if (!force && globalIsDeviceCacheLoaded && globalIsAiModelLoaded) return;
        ...
        try {
            ...
            const response = await fetchApi('/api/device-mapper?action=init_devices_tab');
            if (response.ok) {
                const data = await response.json();
                if (data.cache) {
                    globalCacheData = data.cache;
                    setCacheData(data.cache);
                    globalIsDeviceCacheLoaded = true;
                    setIsDeviceCacheLoaded(true);
                    onDeviceMappingsCountChange(Object.keys(data.cache).length);
                }
    ...
    const executeClearAuditLogs = async () => {
        ...
        try {
            const res = await fetchApi('/api/device-mapper?action=clear_audit_logs', { method: 'POST' });
            if (res.ok) {
                globalAuditLogs = [];
                setAuditLogs([]);
                globalIsAuditLogsLoaded = true;
            ...
            <div className="flex-1 flex flex-col md:flex-row w-full h-full md:min-h-0 overflow-y-auto md:overflow-hidden bg-[var(--dev-console-bg)]">
                ...
                <div className="w-full md:w-[40%] shrink-0 border-b md:border-b-0 md:border-r border-[var(--dev-console-border)] p-4 flex flex-col gap-4 overflow-visible md:overflow-y-auto md:h-full md:min-h-0">
                    ...
                    <form onSubmit={async (e) => {
                        ...
                        try {
                            const response = await fetchApi('/api/device-mapper?action=cache_update', {
                                method: 'POST',
                                headers: { 'Content-Type': 'application/json' },
                        ...
                        <div className="flex gap-2 items-end w-full">
                            ...
                            >
                                {isSaving ? (
                                    <>
                                        <Loader size={12} className="animate-spin text-white shrink-0" />
                                        <span>Saving...</span>
                                    </>
                                ) : (
                                    <span>Save</span>
                                                ...
                                                <div key={model} className="flex justify-between items-center px-4 py-2.5 border-b border-[var(--dev-console-border)] hover:bg-[var(--dev-console-bg-active)] transition-colors text-xs group">
                                                    ...
                                                    <div className="flex items-center gap-1 shrink-0">
                                                        ...
                                                        <InlineConfirmDelete
                                                            onDelete={async () => {
                                                                try {
                                                                    const response = await fetchApi('/api/device-mapper?action=cache_delete', {
                                                                        method: 'POST',
                                                                        headers: { 'Content-Type': 'application/json' },
                            ...
                            <div className="flex-1 overflow-y-auto scrollbar-thin scrollbar-thumb-[var(--dev-console-border)] scrollbar-track-transparent">
                                {isAuditLogsLoading && auditLogs.length === 0 ? (
                                    <div className="p-12 text-center text-xs text-[var(--dev-console-text-muted)] flex flex-col items-center justify-center gap-2">
                                        <Loader size={24} className="animate-spin text-[#007fd4]" />
                                ...
                                <div className="flex-1 flex flex-col mt-2.5">
                                    ...
                                    {isApiKeysLoading && apiKeys.length === 0 ? (
                                        <div className="p-8 text-center text-xs text-[var(--dev-console-text-muted)] flex items-center justify-center gap-2">
                                            <Loader size={14} className="animate-spin text-[#007fd4]" />
                                                                ...
                                                                <div className="flex items-center gap-1 shrink-0 font-sans">
                                                                    ...
                                                                    <button 
                                                                        ...
                                                                        className={`px-2 py-0.5 text-[9px] rounded font-bold font-sans flex items-center gap-1 transition-all border ${
                                                                            isRevokingKey 
                                                                                ? 'bg-red-900/40 text-white/60 border-red-900/30 cursor-not-allowed opacity-75' 
