...
let globalIsSessionCacheLoaded = false;
let globalIsSessionCacheFetching = false;
...
export const useSessions = ({
    ...
    const [isSessionCacheLoaded, setIsSessionCacheLoaded] = useState(() => globalIsSessionCacheLoaded);
    ...
    const fetchSessionCacheData = async (force = false) => {
        if (globalIsSessionCacheFetching) return;
        globalIsSessionCacheFetching = true;
        isFetchingRef.current = true;
        setIsSessionCacheLoading(true);
        window.dispatchEvent(new CustomEvent('session-cache-loading', { detail: { loading: true } }));
        try {
            ...
            if (response.ok && contentType && contentType.includes('application/json')) {
                ...
                globalIsSessionCacheLoaded = true;
            ...
            globalIsSessionCacheFetching = false;
            isFetchingRef.current = false;
            setIsSessionCacheLoading(false);
            window.dispatchEvent(new CustomEvent('session-cache-loading', { detail: { loading: false } }));
        }
    };
