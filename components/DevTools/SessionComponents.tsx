...
const InteractiveStatValue: React.FC<{
    ...
    // Preserve parent props signature safely
    const _dummy = [props.expandedStat, props.setExpandedStat];

    const filteredRef = React.useRef<HTMLSpanElement>(null);
    const allRef = React.useRef<HTMLSpanElement>(null);

    const [hoveredPart, setHoveredPart] = React.useState<'filtered' | 'all' | null>(null);
    const [tappedPart, setTappedPart] = React.useState<'filtered' | 'all' | null>(null);

    const [isFilteredTruncated, setIsFilteredTruncated] = React.useState(false);
    const [isAllTruncated, setIsAllTruncated] = React.useState(false);

    // Global click listener to clear tapped state when clicking outside
    React.useEffect(() => {
        if (!tappedPart) return;
        const handleOutsideClick = () => setTappedPart(null);
        document.addEventListener('click', handleOutsideClick);
        return () => document.removeEventListener('click', handleOutsideClick);
    }, [tappedPart]);

    const activePart = hoveredPart || tappedPart;

    const shouldExpandFiltered = activePart === 'filtered' && isFilteredTruncated;
    const shouldExpandAll = activePart === 'all' && isAllTruncated;

    const handleMouseEnterFiltered = () => {
        if (filteredRef.current) {
            setIsFilteredTruncated(filteredRef.current.scrollWidth > filteredRef.current.clientWidth);
        }
        setHoveredPart('filtered');
    };

    const handleMouseEnterAll = () => {
        if (allRef.current) {
            setIsAllTruncated(allRef.current.scrollWidth > allRef.current.clientWidth);
        }
        setHoveredPart('all');
    };
    ...
    const handleClickFiltered = (e: React.MouseEvent) => {
        ...
        setTappedPart(prev => prev === 'filtered' ? null : 'filtered');
    };

    const handleClickAll = (e: React.MouseEvent) => {
        e.stopPropagation();
        if (allRef.current) {
            setIsAllTruncated(allRef.current.scrollWidth > allRef.current.clientWidth);
        }
        setTappedPart(prev => prev === 'all' ? null : 'all');
    };

    return (
        <div className="relative h-6 w-full min-w-0 flex items-center select-none font-mono">
            <div className="absolute left-0 w-full flex items-baseline leading-none gap-1">
                {/* Filtered Value */}
                <span 
                    ref={filteredRef}
                    onMouseEnter={handleMouseEnterFiltered}
                    onMouseLeave={() => setHoveredPart(null)}
                    onClick={handleClickFiltered}
                    className={`${
                        shouldExpandFiltered 
                            ? valStyle.replace('truncate', '').replace('overflow-hidden', '') 
                            : shouldExpandAll 
                                ? 'hidden' 
                                : valStyle
                    } ${colorClass} cursor-pointer transition-all duration-150`}
                    title={filteredTitle}
                >
                    {filteredVal}
                </span>
                ...
                {/* Separator Slash */}
                <span className={`text-[10px] ${colorClass} opacity-40 font-bold font-sans shrink-0 ${
                    (shouldExpandFiltered || shouldExpandAll) ? 'hidden' : ''
                }`}>
                    /
                </span>
                ...
                <span 
                    ...
                    className={`${
                        shouldExpandAll 
                            ? qValStyle.replace('truncate', '').replace('overflow-hidden', '') 
                            : shouldExpandFiltered 
                                ? 'hidden' 
                                : qValStyle
                    } ${colorClass} opacity-70 cursor-pointer transition-all duration-150`}
                    title={totalTitle}
                >
                    {totalVal}
                </span>
