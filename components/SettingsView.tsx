...
const getRelativeDateAnd24hTime = (dateInput: string | number | Date | null | undefined) => {
  ...
  // Format 24-hour time in Asia/Kolkata timezone exactly like DevConsole
...
const RelativeTimestamp: React.FC<{ dateInput: string | number | Date | null | undefined }> = ({ dateInput }) => {
  ...
  return (
    <span 
      className="cursor-pointer select-none relative group/ts text-gray-900 dark:text-white font-semibold border-b border-dashed border-gray-300 dark:border-white/20 pb-px whitespace-nowrap"
      onClick={(e) => {
        e.stopPropagation();
  ...
  return (
    <>
      <motion.main
        ...
        className="relative z-10 h-full overflow-y-auto bg-gray-50 dark:bg-black transition-colors duration-300 pt-20 md:pt-24 pb-6 dev-console-spacing-pb"
      >
        <div className="w-full max-w-[1600px] mx-auto px-4 sm:px-6 md:px-8 lg:px-12">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6 mt-4 md:mt-2">
            ...
            <motion.div
              variants={itemVariants}
              className={`col-span-1 ${sessions.length > 1 ? 'md:col-span-2' : ''} session-card border rounded-2xl p-4 sm:p-5 shadow-sm hover:shadow-md transition-all flex flex-col`}
            >
              ...
              {!user ? (
                ...
                <div className={sessions.length > 1 ? "grid grid-cols-1 md:grid-cols-2 gap-3" : "space-y-3"}>
                  ...
                  {sessions.length > 1 && (
                    <div className="md:col-span-2 p-3 rounded-xl border session-danger-badge flex items-start gap-2.5 text-xs font-medium">
                        ...
                        <div className="flex flex-col gap-2.5 mt-3 pt-3 border-t border-gray-100/50 dark:border-white/[0.02]">
                          ...
                          <div className="flex flex-row items-center gap-x-2 text-[9px] sm:text-[10px] md:text-[11px] text-gray-500 dark:text-white/40 min-w-0">
                            <span className="flex items-center gap-0.5 shrink-0">
                              ...
                              <span>First: <RelativeTimestamp dateInput={s.created_at} /></span>
                            </span>
                            <span className="text-gray-300 dark:text-white/10 shrink-0 select-none">|</span>
                            <span className="flex items-center gap-0.5 shrink-0">
                              <Clock className="w-3 h-3 text-indigo-400 shrink-0" />
                              <span>Last: <RelativeTimestamp dateInput={s.last_active_at || s.created_at} /></span>
                            </span>
                          </div>
            ...
            <motion.div
              variants={itemVariants}
              className={`col-span-1 ${sessions.length > 1 ? 'md:col-span-2' : ''} border border-gray-200 dark:border-white/10 rounded-3xl p-4 sm:p-5 shadow-sm hover:shadow-md transition-shadow bg-gradient-to-br from-blue-50/40 to-indigo-50/20 dark:from-blue-900/10 dark:to-indigo-900/10 flex flex-col justify-between`}
            >
              <div className="flex items-center gap-3 mb-5">
                <div className="p-2.5 bg-blue-100/80 dark:bg-blue-500/10 rounded-xl text-blue-600 dark:text-blue-400">
              ...
              <div className={`grid ${sessions.length > 1 ? 'grid-cols-2 md:grid-cols-4' : 'grid-cols-2'} gap-2 sm:gap-3 mb-5`}>
                <div className="p-3 bg-white/60 dark:bg-black/20 rounded-2xl border border-gray-200/50 dark:border-white/5 flex flex-col justify-center">
                   <div className="text-[10px] uppercase font-bold text-gray-500 dark:text-white/40 mb-1 flex items-center gap-1"><Monitor className="w-3 h-3"/> Platform</div>
                   <div className="text-xs font-semibold text-gray-800 dark:text-white/80 truncate">{systemInfo.os}</div>
                </div>
                <div className="p-3 bg-white/60 dark:bg-black/20 rounded-2xl border border-gray-200/50 dark:border-white/5 flex flex-col justify-center">
                   <div className="text-[10px] uppercase font-bold text-gray-500 dark:text-white/40 mb-1 flex items-center gap-1"><Globe className="w-3 h-3"/> Engine</div>
                   <div className="text-xs font-semibold text-gray-800 dark:text-white/80 truncate">{systemInfo.browser}</div>
                </div>
                <div className="p-3 bg-white/60 dark:bg-black/20 rounded-2xl border border-gray-200/50 dark:border-white/5 flex flex-col justify-center">
                   <div className="text-[10px] uppercase font-bold text-gray-500 dark:text-white/40 mb-1 flex items-center gap-1"><Layout className="w-3 h-3"/> Display</div>
                   <div className="text-xs font-semibold text-gray-800 dark:text-white/80 truncate">{systemInfo.screen}</div>
                </div>
                <div className="p-3 bg-white/60 dark:bg-black/20 rounded-2xl border border-gray-200/50 dark:border-white/5 flex flex-col justify-center">
                   <div className="text-[10px] uppercase font-bold text-gray-500 dark:text-white/40 mb-1 flex items-center gap-1"><RefreshCw className="w-3 h-3"/> Network</div>
              ...
              <div className="flex flex-row gap-2 sm:gap-3">
                <button
                  ...
                  className="flex-1 flex items-center justify-center py-2.5 px-3 rounded-2xl bg-white/80 dark:bg-white/5 border border-gray-200/50 dark:border-white/10 hover:bg-white dark:hover:bg-white/10 transition-all shadow-sm"
                >
                ...
                <button
                  ...
                  className="flex-1 flex items-center justify-center py-2.5 px-3 rounded-2xl bg-red-50/80 dark:bg-red-500/10 border border-red-100 dark:border-red-500/20 hover:bg-red-100 dark:hover:bg-red-500/20 transition-all text-red-600 dark:text-red-400 shadow-sm"
                >
                  <div className="flex items-center gap-1.5 sm:gap-2">
