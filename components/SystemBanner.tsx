    ...
    useEffect(() => {
        const fetchBanner = async () => {
            try {
                ...
                if (error) {
                    console.warn("SystemBanner fetch error (likely 'type', 'banner_type' or 'is_active' column missing):", error.message);
                    setHasError(true);
                    return;
                }
        ...
        <div className="flex flex-col relative z-50">
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-2 md:py-2 flex items-start sm:items-center justify-between gap-2 sm:gap-3">
                <div className="flex flex-col sm:flex-row sm:items-center flex-1 overflow-hidden gap-1 sm:gap-0 pt-0.5 sm:pt-0">
                    ...
                    <div className={`text-[13px] leading-snug sm:text-sm sm:leading-tight ${activeConfig.colors.desc} ${isCollapsed ? 'line-clamp-1' : ''}`}>
                        {activeConfig.description}
                    </div>
                </div>
                
                <div className={`shrink-0 -mr-1 mt-1 sm:mt-0 self-center transition-transform ${activeConfig.colors.icon}`}>
