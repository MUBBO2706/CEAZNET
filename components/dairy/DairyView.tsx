...
const DairyView: React.FC<DairyViewProps> = ({ setDairyHeaderState, isSuspended }) => {
    ...
    const handleQuickAdd = async (e: React.MouseEvent, item: DairyItem) => {
        ...
        const newEntry: DairyEntry = {
            ...
            isPaid: Boolean(item.isPaidByDefault),
                ...
                {viewMode === 'dashboard' ? (
                    !dataLoaded ? (
                        <div className="flex-grow flex-1 flex flex-col items-center justify-center text-center p-6 my-auto min-h-[450px] animate-in fade-in duration-300">
                            <div className="mb-4">
                                <Loader className="w-10 h-10 text-blue-600 dark:text-blue-500 animate-spin" />
                            </div>
                                ...
                                {items.map((item, index) => {
                                    ...
                                    return (
                                        <div 
                                            ...
                                            className={`group relative rounded-3xl p-6 border transition-colors duration-200 cursor-pointer overflow-hidden ${theme.bg} ${theme.border} ${theme.hoverBorder} select-none`}
                                        >
                                            {/* Blended Background Watermark Icon */}
                                            <div className={`absolute -right-6 -bottom-6 pointer-events-none select-none ${theme.iconColor} opacity-[0.08] dark:opacity-[0.07] group-hover:opacity-[0.14] dark:group-hover:opacity-[0.12] transition-opacity duration-300 transform -rotate-12`}>
                                                {getIcon(item.name, "w-44 h-44 stroke-[1.25]", item.icon)}
                                            </div>
            ...
            {selectedItem && isPaymentModalOpen && (
                <DairyPaymentModal
                    ...
                    onDeletePayment={async (paymentId) => {
                        ...
                        try {
                            await deleteDairyPayment(paymentId, user);
                            if (selectedEntryForPayment) {
