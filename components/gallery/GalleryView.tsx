...
const TelegramMedia: React.FC<{ 
    ...
    viewMode?: 'grid' | 'list'
}> = ({ item, className, onClick, onPointerDown, onPointerUp, onPointerLeave, viewMode = 'grid' }) => {
    ...
    useEffect(() => {
        ...
        const fetchUrl = async (forceFresh = false) => {
            try {
                // Try to get main URL
                const resolvedUrl = await getFileUrlFromTelegram(item.url, false, forceFresh);
                ...
                if (isMounted) {
                    ...
                    if (resolvedUrl === '__TOO_LARGE__') {
                        ...
                        const resolvedThumb = await getFileUrlFromTelegram(item.url, true, forceFresh);
                        if (resolvedThumb && resolvedThumb !== '__NOT_FOUND__') setThumbUrl(resolvedThumb);
                        setLoading(false);
                    } else if (resolvedUrl) {
                        setUrl(resolvedUrl);
                        setIsError(false);
                        ...
                        retryCount++;
                        setTimeout(() => fetchUrl(true), retryDelay);
                    } else {
                        // If main URL fails after retries, try thumb as fallback for grid
                        const resolvedThumb = await getFileUrlFromTelegram(item.url, true, true);
                        if (resolvedThumb && resolvedThumb !== '__NOT_FOUND__') {
    ...
    const handleMediaError = useCallback(() => {
        // If image loading fails (e.g. 1-hour Telegram download URL expired), invalidate cache and re-fetch once
        invalidateTelegramUrlCache(item.url);
        getFileUrlFromTelegram(item.url, false, true).then((freshUrl) => {
            if (freshUrl && freshUrl !== '__NOT_FOUND__' && freshUrl !== '__TOO_LARGE__') {
                setUrl(freshUrl);
                setIsError(false);
            } else {
                setIsError(true);
                    ...
                    {showThumb ? (
                        <>
                            {item.type === 'video' && url && !thumbUrl ? (
                                <video src={url} onError={handleMediaError} className="w-full h-full object-cover opacity-80 group-hover:opacity-100 transition-opacity" preload="metadata" muted playsInline />
                            ) : (
                                <img src={thumbUrl || url || ''} onError={handleMediaError} alt="" className="w-full h-full object-cover" />
                            )}
                            {item.type === 'video' && (
        ...
        const fetchText = async () => {
            try {
                ...
                try {
                    ...
                    const proxyResponse = await fetchApi(PROXY_URL);
...
const PdfViewer: React.FC<{ url: string, filename: string }> = ({ url, filename }) => {
    ...
    useEffect(() => {
        const fetchPdf = async () => {
            try {
                const PROXY_URL = `/api/image-proxy?url=${encodeURIComponent(url)}`;
                const response = await fetchApi(PROXY_URL);
    ...
    return (
        <div className="w-full h-full bg-black flex flex-col items-center animate-in zoom-in-95 duration-500 overflow-hidden relative">
            ...
            `}</style>
            {/* Document Container */}
            <div ref={containerRef} className="flex-1 w-full h-full overflow-auto relative z-0">
...
const GalleryView: React.FC<GalleryViewProps> = ({ user, userProfile, setGalleryHeaderState, isSuspended }) => {
    ...
    const handleDeleteSelected = async () => {
        ...
        try {
            ...
            globalAlert("Failed to delete some items.", { type: 'danger' });
        } finally {
    ...
    const handleUploadTrigger = useCallback(() => {
        if (isSuspended) {
            globalAlert("Upload blocked: Account suspended.", { type: 'danger' });
            return;
        }
            ...
            for (let i = 0; i < newFiles.length; i++) {
                ...
                try {
                    const metadata: UploadMetadata = {
                        userId: user?.id || 'N/A',
                        userName: userProfile?.full_name || userProfile?.name || user?.user_metadata?.full_name || user?.user_metadata?.name || 'Unknown User',
                    ...
                    const tgUrl = await uploadFileToTelegram(file, file.name, metadata);
    ...
    const handleDelete = async (id: string) => {
        ...
        try {
            ...
            globalAlert("Failed to delete item.", { type: 'danger' });
        } finally {
            setDeleting(false);
        }
    };
    ...
    const openFullScreen = async (item: GalleryItem) => {
        ...
        try {
            ...
            if (url === '__NOT_FOUND__') {
                globalAlert('File is temporarily unavailable from Telegram.', { type: 'danger' });
    ...
    const urlId = useMemo(() => {
        ...
        return parts.length >= 3 && parts[1] === 'gallery' && parts[2] ? parts[2] : null;
    }, [location.pathname]);

    const prevGalleryUrlIdRef = useRef<string | null>(urlId);
    if (prevGalleryUrlIdRef.current !== urlId) {
        prevGalleryUrlIdRef.current = urlId;
        if (!urlId) {
            if (selectedItem !== null) setSelectedItem(null);
            if (fullScreenUrl !== null) setFullScreenUrl(null);
        }
    }
    ...
    return (
        <div className="flex flex-col h-full bg-white dark:bg-black">
            ...
            <div className="flex-1 overflow-y-auto py-4 dev-console-spacing-pb">
            ...
            <ConfirmationModal
                ...
                message="This will permanently remove all selected items from your gallery and Telegram."
