...
import { BookOpen, FileText, Wallet, Calendar, Languages, FlaskConical, Mic, Settings, ArrowRight, TrendingUp, TrendingDown, MessageSquare, Clock, Plus, ArrowUpRight, AlertCircle, CheckCircle2, ScrollText, Palette, Volume2, Cpu, ShieldCheck, Database, Layers, Zap, ChevronRight, Sparkles } from 'lucide-react';
...
const HomeView: React.FC<HomeViewProps> = ({ onNavigate, user, userProfile, exploreArticles }) => {
    ...
    const renderHeroSlide = (app: typeof apps[0]) => {
        ...
        const HeroHeader = ({ title, icon: Icon, color, subtitle }: { title: string, icon: any, color: string, subtitle?: string }) => (
            <div className="flex items-center gap-3 mb-4 md:mb-5 relative z-10 shrink-0">
                <div className="p-2 md:p-2.5 rounded-xl bg-white/10 backdrop-blur-md border border-white/20 shadow-md">
                    <Icon className={`w-5 h-5 ${color}`} />
                </div>
                <div className="min-w-0">
                    <h2 className="text-lg md:text-xl lg:text-2xl font-bold text-white tracking-tight leading-none drop-shadow-md truncate">
                        {title}
                    </h2>
                    {subtitle && (
                        <p className="text-[10px] md:text-xs text-white/50 uppercase tracking-widest font-mono mt-1 truncate">{subtitle}</p>
                    )}
                </div>
            </div>
        );
        ...
        if (user) {
            ...
            if (app.id === 'finance' && financeSummary) {
                return (
                    <div className="absolute inset-0 bg-gradient-to-br from-emerald-900 via-emerald-950 to-black p-6 md:p-8 flex flex-col justify-between overflow-hidden">
                        ...
                        <div className="relative z-10 w-full h-full flex flex-col justify-between">
                            ...
                            <div className="flex flex-col gap-3 my-auto">
                                <div>
                                    <p className="text-emerald-200/50 text-[10px] font-bold uppercase tracking-wider mb-1">Total Balance</p>
                                    <p className="text-3xl md:text-4xl font-extrabold text-white tracking-tight">₹{financeSummary.balance.toLocaleString()}</p>
                                </div>
                                <div className="grid grid-cols-2 gap-3 border-t border-white/10 pt-3">
                                    <div>
                                        <p className="text-emerald-200/50 text-[10px] font-bold uppercase tracking-wider mb-0.5">Income</p>
                                        <p className="text-base md:text-lg font-bold text-emerald-400">+₹{financeSummary.income.toLocaleString()}</p>
                                    </div>
                                    <div>
                                        <p className="text-rose-200/50 text-[10px] font-bold uppercase tracking-wider mb-0.5">Expense</p>
                                        <p className="text-base md:text-lg font-bold text-rose-400">-₹{financeSummary.expense.toLocaleString()}</p>
                                    </div>
                                </div>
                            </div>

                            {financeSummary.lastTransaction && (
                                <div className="mt-auto pt-3 border-t border-white/5 inline-flex items-center gap-2 max-w-full">
                                    <div className={`p-1 rounded-full shrink-0 ${financeSummary.lastTransaction.type === 'income' ? 'bg-emerald-500/20 text-emerald-400' : 'bg-rose-500/20 text-rose-400'}`}>
                                        {financeSummary.lastTransaction.type === 'income' ? <ArrowUpRight className="w-3.5 h-3.5" /> : <TrendingDown className="w-3.5 h-3.5" />}
                                    </div>
                                    <div className="flex flex-col min-w-0">
                                        <span className="text-white/40 text-[9px] font-bold uppercase tracking-wider">Last Activity</span>
                                        <div className="flex items-center gap-1.5 overflow-hidden">
                                            <span className="text-white/95 font-semibold text-xs truncate">{financeSummary.lastTransaction.description}</span>
                                            <span className={`text-xs font-bold shrink-0 ${financeSummary.lastTransaction.type === 'income' ? 'text-emerald-400' : 'text-rose-400'}`}>
                                                ₹{financeSummary.lastTransaction.amount}
                                            </span>
                                        </div>
                                    </div>
                                </div>
            ...
            if (app.id === 'notes' && recentNotes.length > 0) {
                ...
                return (
                    <div className="absolute inset-0 bg-gradient-to-br from-amber-900 via-amber-950 to-black p-6 md:p-8 flex flex-col justify-between overflow-hidden">
                        ...
                        <div className="relative z-10 w-full h-full flex flex-col justify-between">
                            ...
                            <div className="bg-white/5 border border-white/10 rounded-2xl p-4 shadow-xl relative group my-auto">
                                <div className="absolute -top-2.5 left-6 w-10 h-3 bg-amber-500/30 rotate-[-2deg]" />
                                <h3 className="text-base md:text-lg font-bold text-white mb-1.5 font-serif leading-tight truncate">{note.title || 'Untitled Note'}</h3>
                                <p className="text-xs md:text-sm text-amber-100/70 line-clamp-3 font-mono leading-relaxed">
                                    {plainContent || 'No content'}
                                </p>
                            </div>

                            <div className="mt-auto pt-3 border-t border-white/5 flex items-center justify-between text-[9px] text-amber-200/40 uppercase tracking-wider font-bold">
                                <span className="flex items-center gap-1">
                                    <Clock className="w-3 h-3" />
                                ...
                                <div className="flex gap-1">
                                    {recentNotes.map((_, idx) => (
                                        <div key={idx} className={`w-1 h-1 rounded-full ${idx === currentNoteIndex ? 'bg-amber-400 w-3' : 'bg-white/20'}`} />
                                    ))}
                                </div>
                            </div>
                        </div>
            ...
            if (app.id === 'dairy' && dairySummary) {
                ...
                return (
                    <div className="absolute inset-0 bg-neutral-900 p-6 md:p-8 flex flex-col justify-between overflow-hidden">
                        ...
                        <div className="relative z-10 w-full h-full flex flex-col justify-between">
                            ...
                            <div className="grid grid-cols-2 gap-3 my-auto">
                                <div className="p-3 rounded-xl bg-white/5 border border-white/10 relative overflow-hidden group">
                                    <p className="text-neutral-400 text-[10px] font-bold uppercase tracking-wider mb-0.5">Outstanding Due</p>
                                    <p className="text-base md:text-lg font-mono font-bold text-red-400 tracking-tight truncate">₹{dairySummary.due.toLocaleString()}</p>
                                    <div className="mt-1 flex items-center gap-1 text-[9px] font-medium text-red-400 bg-red-500/10 px-1.5 py-0.5 rounded-md w-fit">
                                ...
                                <div className="p-3 rounded-xl bg-white/5 border border-white/10 relative overflow-hidden group">
                                    <p className="text-neutral-400 text-[10px] font-bold uppercase tracking-wider mb-0.5">Total Paid</p>
                                    <p className="text-base md:text-lg font-mono font-bold text-emerald-400 tracking-tight truncate">₹{dairySummary.paid.toLocaleString()}</p>
                                    <div className="mt-1 flex items-center gap-1 text-[9px] font-medium text-emerald-400 bg-emerald-500/10 px-1.5 py-0.5 rounded-md w-fit">
                            ...
                            <div className="mt-auto pt-3 border-t border-white/5 shrink-0">
                                <div className="flex justify-between items-end mb-1">
                                    <span className="text-[9px] font-bold text-neutral-400 uppercase tracking-wider">Payment Progress</span>
            ...
            if (app.id === 'molecule-viewer' && lastMolecule) {
                return (
                    <div className="absolute inset-0 bg-gradient-to-br from-indigo-900 via-indigo-950 to-black p-6 md:p-8 flex flex-col justify-between overflow-hidden">
                        ...
                        <div className="relative z-10 w-full h-full flex flex-col justify-between">
                            ...
                            <div className="flex items-center gap-4 my-auto min-h-0">
                                <div className="flex-1 min-w-0">
                                    <div className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full bg-indigo-500/20 border border-indigo-500/30 text-indigo-300 text-[10px] font-bold uppercase tracking-wider mb-2">
                                        <span className="w-1.5 h-1.5 rounded-full bg-indigo-400 animate-pulse" />
                                        Last Analyzed
                                    </div>
                                    <h3 className="text-xl md:text-2xl font-extrabold text-white leading-tight mb-3 truncate font-serif italic">
                                        {lastMolecule}
                                    </h3>
                                    ...
                                    {moleculeData && (
                                        <div className="grid grid-cols-2 gap-2">
                                            <div className="p-2.5 rounded-xl bg-white/5 border border-white/5">
                                                <p className="text-indigo-200/50 text-[9px] font-bold uppercase mb-0.5 truncate">Formula</p>
                                                <p className="text-xs font-bold text-white font-mono truncate">{moleculeData.molecularFormula || 'N/A'}</p>
                                            </div>
                                            <div className="p-2.5 rounded-xl bg-white/5 border border-white/5">
                                                <p className="text-indigo-200/50 text-[9px] font-bold uppercase mb-0.5 truncate">Weight</p>
                                                <p className="text-xs font-bold text-white font-mono truncate">{moleculeData.molecularWeight ? `${moleculeData.molecularWeight}` : 'N/A'}</p>
                                            </div>
                                        </div>
                                    )}
                                </div>
                                ...
                                <div className="relative w-20 h-20 md:w-24 md:h-24 bg-black/40 rounded-2xl border border-white/10 flex items-center justify-center shrink-0 shadow-xl overflow-hidden">
                                    <div className="w-12 h-12 relative animate-[spin_15s_linear_infinite]">
                                        ...
                                        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-3.5 h-3.5 bg-indigo-500 rounded-full" />
                            ...
                            <div className="mt-auto pt-3 border-t border-white/5 text-[9px] text-indigo-300/40 font-mono uppercase tracking-wider">
            ...
            if (app.id === 'settings' && settingsSummary) {
                return (
                    <div className="absolute inset-0 bg-neutral-950 p-6 md:p-8 flex flex-col justify-between overflow-hidden">
                            ...
                            <div className="flex flex-col gap-2.5 my-auto">
                                <div className="flex items-center justify-between p-3 rounded-xl bg-white/5 border border-white/10">
                                    <div className="flex items-center gap-2">
                                        ...
                                        <span className="text-xs font-bold text-neutral-300">Theme</span>
                                    </div>
                                    <span className="text-xs font-mono text-emerald-400 capitalize">{settingsSummary.theme}</span>
                                </div>
                                <div className="flex items-center justify-between p-3 rounded-xl bg-white/5 border border-white/10">
                                    <div className="flex items-center gap-2">
                                        <Mic className="w-4 h-4 text-slate-400" />
                                        <span className="text-xs font-bold text-neutral-300">Voice Assistant</span>
                                    </div>
                                    <span className="text-xs font-mono text-blue-400 capitalize">{settingsSummary.voice}</span>
                                </div>
                            </div>

                            <div className="mt-auto pt-3 border-t border-white/5 text-[9px] text-slate-400 font-mono uppercase tracking-wider">
            ...
            if (app.id === 'live-conversation' && recentConversation) {
                return (
                    <div className="absolute inset-0 bg-gradient-to-br from-rose-900 via-rose-950 to-black p-6 md:p-8 flex flex-col justify-between overflow-hidden">
                        ...
                        <div className="relative z-10 w-full h-full flex flex-col justify-between">
                            ...
                            <div className="space-y-3 my-auto">
                                <div className="bg-white/5 border border-white/10 rounded-2xl rounded-tl-none p-4 max-w-2xl">
                                    <p className="text-sm md:text-base text-white font-medium leading-relaxed line-clamp-3">
                                        "{recentConversation.title || 'No title'}"
                                    </p>
                                </div>
                            </div>

                            <div className="flex gap-2.5 mt-auto pt-3 border-t border-white/5 items-center justify-between text-[9px] font-mono text-rose-300/60 uppercase">
                                <span className="flex items-center gap-1">
                                    <Clock className="w-3 h-3" />
                                    {new Date(recentConversation.createdAt).toLocaleDateString()}
                                </span>
                        ...
                        <div className="absolute inset-0 p-6 md:p-8 flex flex-col justify-end">
                            <div className="relative z-10 w-full">
                                <div className="flex items-center gap-2 mb-2">
                                    <span className="px-2 py-0.5 rounded-full bg-blue-600 text-white text-[9px] font-bold uppercase tracking-wider">Trending News</span>
                                    <span className="text-gray-300 text-[9px] font-bold uppercase tracking-wider truncate max-w-[150px]">
                                        {article.source.name}
                                    </span>
                                </div>
                                <h2 className="text-lg md:text-xl font-bold text-white mb-2 leading-tight line-clamp-2 drop-shadow-md">
                                    {article.title}
                                </h2>
                                <p className="text-xs md:text-sm text-gray-200 line-clamp-2 drop-shadow-sm font-medium">
                    ...
                    <div className="absolute inset-0 bg-gradient-to-br from-indigo-900 via-indigo-950 to-black p-6 md:p-8 flex flex-col justify-between overflow-hidden">
                        ...
                        <div className="relative z-10 w-full h-full flex flex-col justify-between">
                            ...
                            <div className="my-auto flex flex-col gap-1.5">
                                <span className="text-4xl md:text-5xl font-extrabold text-white tracking-tighter leading-none">
                                    {(translatorStats.input + translatorStats.output).toLocaleString()}
                                </span>
                                <span className="text-[10px] text-indigo-300 font-bold uppercase tracking-wider">characters processed</span>
                            </div>

                            <div className="grid grid-cols-2 gap-2 mt-auto pt-3 border-t border-white/5">
                                <div className="p-2.5 rounded-xl bg-white/5 border border-white/5">
                                    <p className="text-indigo-200/50 text-[9px] font-bold uppercase mb-0.5 truncate">Input</p>
                                    <p className="text-sm font-bold text-white truncate">{translatorStats.input.toLocaleString()}</p>
                                </div>
                                <div className="p-2.5 rounded-xl bg-white/5 border border-white/5">
                                    <p className="text-indigo-200/50 text-[9px] font-bold uppercase mb-0.5 truncate">Output</p>
                                    <p className="text-sm font-bold text-white truncate">{translatorStats.output.toLocaleString()}</p>
                                </div>
                            </div>
                        </div>
                    </div>
        ...
        return (
            <div className="absolute inset-0 overflow-hidden">
                ...
                <div className="absolute inset-0 p-6 md:p-8 flex flex-col justify-between relative z-10">
                    <HeroHeader title={app.title} icon={app.icon} color="text-white" subtitle="Digital Workspace App" />
                    
                    <div className="my-auto">
                        <p className="text-sm md:text-base text-gray-200 font-medium leading-relaxed drop-shadow-md line-clamp-3 max-w-xl">
                            {app.description}
                        </p>
                    </div>

                    <div className="mt-auto pt-3 border-t border-white/5 flex items-center justify-between">
                        <button className="flex items-center gap-2 px-5 py-2.5 bg-white text-black font-bold rounded-full hover:bg-gray-100 hover:scale-105 active:scale-95 transition-all duration-300 text-xs shadow-lg group/btn shrink-0">
                            Open App <ArrowRight className="w-3.5 h-3.5 group-hover/btn:translate-x-1 transition-transform" />
                        </button>
                        <span className="text-[10px] text-white/40 uppercase tracking-widest font-mono hidden sm:inline">Offline Persistent</span>
                    </div>
                </div>
            </div>
        );
    ...
    return (
        <div className="px-3 py-6 pt-20 md:px-6 md:py-12 md:pt-24 w-full max-w-[1600px] mx-auto h-full overflow-y-auto scrollbar-hide dev-console-spacing-pb">
            ...
            {/* Mobile View: Single card, same as original */}
            <div className="md:hidden mb-8 w-full">
                <div 
                    className="relative w-full h-[18rem] rounded-[2rem] overflow-hidden shadow-lg group cursor-pointer" 
