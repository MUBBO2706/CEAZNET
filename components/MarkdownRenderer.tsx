...
const MarkdownRenderer: React.FC<MarkdownRendererProps> = ({ content, sources, isStreaming, setCodeForPreview }) => {
    ...
    const flushList = (key: string | number) => {
        if (currentList) {
            ...
            } list-inside space-y-1 my-1.5 pl-5`;
            elements.push(
    ...
    while (i < lines.length) {
        ...
        // HTML Blocks (e.g. Finance Widgets, Comment tags, Headings, blockquotes)
        // Detects lines starting with comments, <div, <blockquote, <h1, <h3, or <hr and renders them as raw HTML
        ...
        if (
            ...
            elements.push(
                <div key={i} className="my-0" dangerouslySetInnerHTML={{ __html: line }} />
            );
            i++;
            continue;
        }
        ...
        if (line.match(/^(---|___|\*\*\*)\s*$/)) {
            ...
            elements.push(<hr key={i} className="my-3 border-0 border-t-[0.5px] border-neutral-200/50 dark:border-white/10" />);
            i++;
            continue;
        }
