...
const ChemistryView: React.FC<ChemistryViewProps> = ({ customMolecule, user }) => {
    ...
    const fetchFromPubChem = async (name: string, shouldSave: boolean = true, initialSettings?: MoleculeViewerState) => {
        ...
        setError(null);
        try {
            let structureRes = await fetchApi(`https://pubchem.ncbi.nlm.nih.gov/rest/pug/compound/name/${encodeURIComponent(name)}/record/JSON?record_type=3d`);
            if (!structureRes.ok) {
                structureRes = await fetchApi(`https://pubchem.ncbi.nlm.nih.gov/rest/pug/compound/name/${encodeURIComponent(name)}/record/JSON?record_type=2d`);
            }
