...
const VoiceHistoryView: React.FC<VoiceHistoryViewProps> = ({ onBack, user, userProfile, isSaving, showBackButton, onOpenSidebar, searchQuery, onNavigate, onLogout, onOpenAuthModal, onOpenProfileModal, isSuspended }) => {
    ...
    useEffect(() => {
        const fetchHistory = async () => {
            ...
            // If we already have cached conversations for this user, we don't need to fetch again
            if (cachedConversations && prevUserId.current === user.id) {
                setIsLoading(false);
                setDataLoaded(true);
                return;
            }
            ...
            try {
                // If there's already an active fetch promise, wait for/re-use it to prevent concurrent identical fetches
        ...
        // If user changed or a save completed, completely invalidate the global cache
        if (userChanged || saveFinished) {
        ...
        if (!dataLoaded || userChanged || saveFinished) {
            fetchHistory();
            prevUserId.current = user?.id;
        }

        prevIsSaving.current = isSaving;
    }, [user?.id, isSaving, dataLoaded]);
    ...
    const confirmDelete = async () => {
        if (isSuspended) {
            alert("Delete blocked: Account suspended.");
            return;
        }
