...
import { LogOut, UserCog, LogIn, X, Bookmark, LoaderCircle, Search, Eye, Heart, ArrowLeft, Trash2, Upload, Download, RotateCw, Edit2, Check, HeadphonesIcon } from 'lucide-react';
                ...
                <div className={`pointer-events-auto flex items-center gap-1 p-1 bg-white/80 dark:bg-black/60 backdrop-blur-xl border border-black/5 dark:border-white/10 rounded-full shadow-sm transition-all duration-300 ${isSearchExpanded ? 'w-full flex-1' : ''}`}>
                    ...
                    {!isSearchExpanded && (
                        <>
                            ...
                            {(isHomeView || isExploreView || isNotesView || isFinanceView || isDairyView || isGalleryView || (isVoiceHistoryView && !props.expandedVoiceTitle) || isMoleculeView || isSupportView || isVoiceSettingsView || (isProfileView && props.supportHeaderState?.title)) && (
