...
import { LogOut, UserCircle2, LogIn, X, Bookmark, LoaderCircle, Search, Eye, Heart, ArrowLeft, Trash2, Upload, Download, RotateCw, Edit2, Check, HeadphonesIcon } from 'lucide-react';
        ...
        >
            ...
            <div className="p-2 space-y-1">
                ...
                <button
                    ...
                    className="w-full flex items-center gap-3 p-2.5 text-sm font-medium text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-xl transition-all group disabled:opacity-60 disabled:cursor-not-allowed"
                >
                    ...
                    {props.notesHeaderState.isWalletLinked && (
                        <div className="pointer-events-auto flex items-center justify-center p-1 bg-white/80 dark:bg-black/60 backdrop-blur-xl border border-black/5 dark:border-white/10 rounded-full shadow-sm transition-all duration-300">
                            <button
                                ...
                                title="Sync Wallet"
                            >
                                {props.notesHeaderState.isSyncing ? <LoaderCircle className="h-5 w-5 animate-spin" /> : <RotateCw className="h-5 w-5" />}
                            </button>
                        </div>
                    )}
                    <div className="pointer-events-auto flex items-center justify-center p-1 bg-white/80 dark:bg-black/60 backdrop-blur-xl border border-black/5 dark:border-white/10 rounded-full shadow-sm transition-all duration-300">
                        {props.notesHeaderState.isReadOnly ? (
                            ...
                            >
                                {props.notesHeaderState.isSaving ? (
                                    <LoaderCircle className="h-5 w-5 animate-spin text-emerald-600 dark:text-emerald-400" />
                                ) : (
                            ...
                            {isVoiceSettingsView && props.onSaveVoiceSettings && (
                                <div className="relative">
                                    <button 
                                        ...
                                        title="Save Settings"
                                    >
                                        {props.isSavingVoiceSettings ? <LoaderCircle className="h-5 w-5 animate-spin" /> : <Check className="h-5 w-5" />}
                                    </button>
                                </div>
                            )}

                            {(isHomeView || isExploreView || isNotesView || isFinanceView || isDairyView || isGalleryView || (isVoiceHistoryView && !props.expandedVoiceTitle) || isMoleculeView || isSupportView || isVoiceSettingsView) && (
