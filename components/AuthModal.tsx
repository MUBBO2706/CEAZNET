...
const AuthModal: React.FC<AuthModalProps> = ({ isOpen, onClose }) => {
    ...
    const autoGenerate = () => {
        ...
        const capitalizedBase = basePart ? basePart.charAt(0).toUpperCase() + basePart.slice(1).toLowerCase() : 'User';
        const chars = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789!@#$%^&*()_+~|}{[]:;?><,./-=";
        let randomPart = "";
        for (let i = 0; i < 8; i++) {
            randomPart += chars[Math.floor(Math.random() * chars.length)];
        }
        const pass = `${capitalizedBase}!1${randomPart}`;
        setPassword(pass);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            if (isForgotPasswordView) {
                ...
                const generatedAvatarUrl = `https://api.dicebear.com/8.x/bottts/svg?seed=${encodeURIComponent(email)}`;
    ...
    return (
        <div 
            className={`fixed inset-0 z-[60] overflow-y-auto transition-all duration-300 ${isOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}
        >
            {/* Deep Dark/Light Backdrop with Blur */}
            <div 
                className="fixed inset-0 bg-[#0a0a0a]/60 dark:bg-black/80 backdrop-blur-xl transition-opacity" 
                onClick={handleClose}
            />
            ...
            <div className="flex min-h-full items-center justify-center p-4">
                <div 
                    className={`relative w-full max-w-[400px] my-auto transform transition-all duration-500 cubic-bezier(0.34, 1.56, 0.64, 1) ${isOpen ? 'scale-100 translate-y-0 opacity-100' : 'scale-95 translate-y-8 opacity-0'}`}
                    ...
                    <div className="px-8 pt-10 pb-8">
                        ...
                        {isVerificationPending ? (
                            ...
                            {!isForgotPasswordView && (
                                <>
                                    ...
                                    {!isLoginView && password && (
                                        <div className="space-y-2 animate-fade-in-up px-1">
                                            <div className="flex justify-between items-center text-xs font-medium">
                                                <span className="text-neutral-500 dark:text-gray-400">Strength</span>
                                    ...
                                    {!isLoginView && (
                                        <div className="flex justify-end px-1 pt-1">
                                            <button 
                                                ...
                                                disabled={!email.trim()}
                                                className={`text-xs font-semibold transition-colors ${!email.trim() ? 'text-neutral-400 cursor-not-allowed opacity-60' : 'text-amber-600 dark:text-amber-500 hover:text-amber-700 dark:hover:text-amber-400'}`}
                                            >
                                                ✨ Generate password
                                            </button>
                                        </div>
                            ...
                            {isLoginView && !isForgotPasswordView && (
                                <div className="flex justify-end pt-0.5 mb-1">
                                    <button 
                                        ...
                                        className="text-xs text-neutral-500 hover:text-neutral-900 dark:text-gray-400 dark:hover:text-white font-medium transition-colors"
                                    >
                                        Forgot Password?
                                    </button>
                                </div>
                            ...
                            <button
                                ...
                                className="group relative w-full flex items-center justify-center gap-2 py-3.5 bg-neutral-900 dark:bg-white text-white dark:text-black font-bold rounded-full overflow-hidden transition-all hover:shadow-xl hover:shadow-amber-500/20 active:scale-[0.98] disabled:opacity-70 disabled:cursor-not-allowed disabled:hover:shadow-none mt-2"
                            >
                                <div className="absolute inset-0 w-full h-full bg-gradient-to-r from-transparent via-white/20 to-transparent -translate-x-[100%] group-hover:translate-x-[100%] transition-transform duration-1000" />
                                {loading ? (
                                    <LoaderCircle className="h-5 w-5 animate-spin" />
                                ...
                                <>
                                    <div className="relative my-4">
                                        ...
                                        <div className="relative flex justify-center text-xs">
                                            <span className="px-2 bg-white dark:bg-black text-neutral-400 dark:text-gray-500">Or continue with</span>
                                        </div>
                                    </div>
                                    <div className="flex justify-center gap-4 mt-3">
                                        <button
                                            ...
                                            className="flex items-center justify-center w-12 h-12 rounded-full bg-neutral-50 hover:bg-neutral-100 dark:bg-white/5 dark:hover:bg-white/10 text-neutral-900 dark:text-white transition-all border border-neutral-200 dark:border-white/10 active:scale-95 shadow-sm"
