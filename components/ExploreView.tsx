...
const ExploreView: React.FC<ExploreViewProps> = ({
    ...
    const handleNext = useCallback(() => {
        if (articles.length > 0) {
            if (currentIndex < articles.length - 1) {
                ...
                // Reached the end of category -> smoothly transition to next category
                ...
                setCurrentIndex(() => 0);
                navigate(nextCategory === 'for-you' ? '/explore/for-you' : '/explore/' + nextCategory);
            }
        }
    }, [articles.length, currentIndex, activeCategory, setActiveCategory, setCurrentIndex, navigate]);
    ...
    const handlePointerUpOrLeave = (e: React.PointerEvent<HTMLDivElement>) => {
        if (activeCategory === 'for-you') {
            ...
            if (Math.abs(finalDragX) > SWIPE_THRESHOLD) {
                if (finalDragX < 0) { // Swipe left -> Next
                    if (currentIndex < articles.length - 1) {
                        setCurrentIndex(prev => prev + 1);
                    } else {
                        // Swiped past the last card in this category -> Go to next category
                        const currentCatIdx = categories.indexOf(activeCategory);
                        const nextCategory = categories[(currentCatIdx + 1) % categories.length];
                        setActiveCategory(nextCategory);
                        setCurrentIndex(() => 0);
    ...
    const handleUnbookmarkInSheet = useCallback((articleUrl: string) => {
         handleInteraction(articleUrl, 'bookmark', false);
    }, [handleInteraction]);

    return (
        <main 
            className={`h-full flex flex-col bg-pan-animation transition-all duration-300 ${isDesktop ? 'overflow-y-auto' : 'overflow-hidden'}`}
            style={{
                ...backgroundStyle,
        ...
        >
                            ...
                            return (
                                <button
                                    ...
                                    onClick={() => {
                                        ...
                                        setCurrentIndex(() => 0);
                                        navigate(category === 'for-you' ? '/explore/for-you' : '/explore/' + category);
                                    }}
                                    className={`
                                        group relative px-4 py-2 md:px-5 md:py-2.5 rounded-full 
                                        flex items-center gap-2 transition-all duration-300 ease-out border
                                        ${isActive 
                                            ? 'bg-[var(--category-tab-active-bg)] text-[var(--category-tab-active-text)] shadow-[var(--category-tab-active-shadow)] scale-105 font-bold border-transparent' 
                                            : 'bg-[var(--category-tab-inactive-bg)] text-[var(--category-tab-inactive-text)] border-[var(--category-tab-inactive-border)] hover:bg-[var(--category-tab-inactive-hover-bg)] hover:text-[var(--category-tab-inactive-hover-text)] hover:scale-105 backdrop-blur-md'
                                        }
                                    `}
                                >
            ...
            >
                <div className="relative w-full h-full flex items-center justify-center max-w-md mx-auto px-4">
                    ...
                    {!isLoading && !error && articles.length > 0 && currentIndex >= articles.length && (
                         <div className="text-center p-4">
                            <h2 className="text-xl font-semibold text-white mb-2">You've reached the end!</h2>
                            <button 
                                onClick={() => {
                                    const currentCatIdx = categories.indexOf(activeCategory);
                                    const nextCategory = categories[(currentCatIdx + 1) % categories.length];
                            ...
                            >
                                ...
                                Next: {(() => {
                                    const currentCatIdx = categories.indexOf(activeCategory);
                                    const nextCategory = categories[(currentCatIdx + 1) % categories.length];
                                    return nextCategory === 'for-you' ? 'For You' : nextCategory.charAt(0).toUpperCase() + nextCategory.slice(1);
                                })()}
                            </button>
                        </div>
                    )}
            ...
            <div className="hidden md:block flex-1 overflow-y-auto px-6 pb-10" style={{ paddingBottom: 'calc(var(--dev-console-padding, 0px) + 2.5rem)' }}>
                {isLoading ? (
                    <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 w-full max-w-[1600px] mx-auto">
                        {[...Array(6)].map((_, i) => (
                    ...
                    <motion.div 
                        className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 w-full max-w-[1600px] mx-auto"
                        initial="hidden"
                        animate="show"
                        variants={{
                            hidden: { opacity: 0 },
