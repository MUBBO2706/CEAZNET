...
const TransactionModalComponent: React.FC<TransactionModalProps> = ({ 
    ...
    useEffect(() => {
        if (initialData && isOpen) {
            const rawCat = (initialData.category || '').trim();
            if (!rawCat) return;
            ...
            const matchedStandard = allStandard.find(matchesCategory);
            const matchedCustom = customCategories.find(matchesCategory);
            
            if (matchedStandard || matchedCustom) {
                setIsCustomCategory(false);
                if (matchedStandard && matchedStandard.id !== category) {
                    setCategory(matchedStandard.id);
                } else if (matchedCustom && matchedCustom.id !== category) {
                    setCategory(matchedCustom.id);
                }
            } else {
                            ...
                            <div className={`space-y-2 rounded-2xl transition-all ${
                                ...
                                <div className="flex justify-between items-center px-1">
                                    <div className="flex items-center">
                                        ...
                                        {category ? (
                                            <span className="text-indigo-600 dark:text-indigo-400 font-bold text-xs ml-1.5">• {getCategoryConfig(category, type, customCategories)?.label || category.replace(/_/g, ' ')}</span>
                                ...
                                {isCustomCategory ? (
                                    ...
                                    <div ref={categoryListRef} className="flex overflow-x-auto gap-2 py-1 px-1 no-scrollbar">
                                        {visibleCategories.map((cat, idx) => renderCategoryButton(cat, false, `vis-${idx}-`))}
                        ...
                        {!categorySearchQuery && recentTypeCategories.length > 0 && (
                            <div className="mb-6">
                                ...
                                <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-3">
                                    {recentTypeCategories.map(cat => renderCategoryButton(cat, true, 'rec-'))}
                                </div>
                            </div>
                        )}
                        ...
                        <div>
                            ...
                            {filteredCategories.length > 0 ? (
                                <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-3 content-start">
                                    {filteredCategories.map(cat => renderCategoryButton(cat, true, 'all-'))}
                                </div>
                            ) : (
                            ...
                            <button
                                ...
                                onClick={async () => {
                                    ...
                                    if (aiSuggestion.isExisting) {
                                        const targetCatId = aiSuggestion.categoryId || aiSuggestion.categoryName;
