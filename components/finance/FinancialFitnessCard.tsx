...
const FinancialFitnessCard: React.FC<FinancialFitnessCardProps> = ({ 
    ...
    const stats = useMemo(() => {
        const now = new Date();
        let daysInPeriod = daysInPeriodProp || 30;
        let daysPassed = 0;
        let daysRemaining = 0;
        ...
        if (period === 'this-month') {
            ...
            // All Time - Prefer provided global daysInPeriod, fallback to active days, else estimate
            if (daysInPeriodProp && daysInPeriodProp > 1) {
                daysPassed = daysInPeriodProp;
                daysInPeriod = daysInPeriodProp;
            } else if (activeExpenseDaysProp && activeExpenseDaysProp > 1) {
                daysPassed = activeExpenseDaysProp;
                daysInPeriod = activeExpenseDaysProp;
        ...
        // Calculate totals from passed explicit props or fallback to transactions
        const income = incomeProp !== undefined 
            ? incomeProp 
            : transactions.filter(t => t.type === 'income').reduce((sum, t) => sum + Number(t.amount), 0);
        const expense = expenseProp !== undefined 
            ? expenseProp 
    ...
    }, [transactions, budget, period, incomeProp, expenseProp]);

    return (
        <div className={`w-full h-full flex flex-col justify-between p-3.5 sm:p-4 rounded-2xl bg-white dark:bg-black border border-gray-200/80 dark:border-gray-800/80 shadow-sm hover:shadow-md transition-all gap-2.5 ${className || ''}`}>
            ...
            {/* Footer: Hard Numbers */}
            <div className="flex justify-between items-center pt-2 mt-0.5 border-t border-gray-100 dark:border-gray-800">
                <div>
