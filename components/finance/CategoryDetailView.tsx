...
export const CategoryDetailView: React.FC<CategoryDetailViewProps> = ({
    ...
    }, [categoryId, categoryType, isNew, user, walletId]);
    ...
    const isCustom = useMemo(() => {
        ...
        const lower = categoryId.toLowerCase().trim();
        const norm = lower.replace(/[_\s-]+/g, ' ');
        return customCategories.some(c => {
            const cId = c.id.toLowerCase().trim();
            const cLabel = (c.label || '').toLowerCase().trim();
            return cId === lower || cLabel === lower ||
                   cId.replace(/[_\s-]+/g, ' ') === norm ||
                   cLabel.replace(/[_\s-]+/g, ' ') === norm;
        });
