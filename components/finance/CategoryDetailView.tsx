...
export const CategoryDetailView: React.FC<CategoryDetailViewProps> = ({
    ...
    }, [isNew, label, handleDetailBack, setCategoryHeaderState]);
        ...
        const loadCategory = async () => {
            ...
            try {
                ...
                if (config) {
                    ...
                    setIconName((config as any).iconName || 'Tag');
    ...
    const stats = useMemo(() => {
        ...
        if (impact?.totalAmount !== undefined && total === 0) {
        ...
        const grandTotal = transactions.reduce((sum, t) => sum + Math.abs(Number(t.amount || 0)), 0);
        ...
        const categoryRank = rankIndex !== -1 ? rankIndex + 1 : 1;
    ...
    }, [impact, matchingTransactions, transactions, categoryId]);
    ...
    const handleReassign = async () => {
        ...
        try {
            await reassignCategoryTransactions(categoryId, targetCategory, user);
            globalAlert(`Successfully moved transactions to "${targetCategory}".`);
            // Refresh impact
            const updatedImpact = await getCategoryUsageImpact(categoryId, user);
    ...
    if (isLoading) {
        return (
            <div className="relative z-10 h-full overflow-y-auto bg-transparent scrollbar-hide pt-16 sm:pt-20 pb-20 flex flex-col items-center justify-center font-sans">
    ...
    return (
        <div 
            className="relative z-10 h-full overflow-y-auto bg-transparent scrollbar-hide pt-16 sm:pt-20 pb-0 font-sans"
            style={{
        ...
        >
            <div className="max-w-4xl mx-auto px-3.5 sm:px-6 lg:px-8 space-y-4 sm:space-y-6 pb-0">
                ...
                {/* Live Identity Hero & Quick Action Bar (Compact & Responsive) */}
                <div className="p-3.5 sm:p-4 rounded-2xl bg-[var(--cat-card-bg)] border border-[var(--cat-card-border)] flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 shadow-xs">
                    <div className="flex items-center gap-3 min-w-0">
                        <div className={`w-11 h-11 sm:w-12 sm:h-12 rounded-xl flex items-center justify-center shrink-0 shadow-xs transition-all duration-300 ${bg}`}>
                            <LiveIconComponent className={`w-5 h-5 sm:w-6 sm:h-6 ${color}`} />
                        </div>
                        <div className="min-w-0">
                            <div className="flex items-center gap-1.5 flex-wrap">
                                ...
                                <span className="text-[9px] font-bold uppercase tracking-wider px-1.5 py-0.2 rounded-md bg-indigo-100 dark:bg-indigo-950/60 text-indigo-700 dark:text-indigo-300">
                                    {type}
                                </span>
                                {isCustom && (
                                    <span className="text-[9px] font-bold uppercase tracking-wider px-1.5 py-0.2 rounded-md bg-emerald-100 dark:bg-emerald-950/60 text-emerald-700 dark:text-emerald-300">
                                        Custom
                                    </span>
                                )}
                            </div>
                    ...
                    {/* Action Buttons */}
                    <div className="flex items-center gap-2 w-full sm:w-auto justify-end shrink-0 pt-2 sm:pt-0 border-t sm:border-t-0 border-gray-100 dark:border-white/[0.06]">
                        {!isNew && (
                            <button
                                type="button"
                                onClick={handleDelete}
                                disabled={isDeleting}
                                className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold text-rose-600 dark:text-rose-400 hover:bg-rose-50 dark:hover:bg-rose-950/30 rounded-xl transition-colors border border-rose-200/80 dark:border-rose-900/40 cursor-pointer"
                            >
                                {isDeleting ? (
                                ...
                                <span>{isCustom ? 'Delete' : 'Hide'}</span>
                            </button>
                        )}
                        ...
                        <button
                            ...
                            className="inline-flex items-center gap-1.5 px-4 py-1.5 text-xs font-bold text-white bg-indigo-600 hover:bg-indigo-700 active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed rounded-xl shadow-xs transition-all cursor-pointer"
                        >
                            {isSaving ? (
                                <Loader className="w-3.5 h-3.5 animate-spin" />
                            ) : (
                ...
                <div className="space-y-3 p-3.5 sm:p-4 rounded-2xl bg-[var(--cat-card-bg)] border border-[var(--cat-card-border)]">
                    ...
                    <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
                        ...
                        <div className="flex-1 space-y-1 min-w-0">
                            ...
                            <input
                                ...
                                className="w-full px-3 py-1.5 text-xs sm:text-sm rounded-xl border border-gray-200 dark:border-white/10 bg-white/80 dark:bg-white/[0.04] text-gray-900 dark:text-white placeholder-gray-400 focus:outline-none focus:border-indigo-500 transition-all font-medium h-[38px] sm:h-[34px]"
                            />
                        </div>
                ...
                {/* Section 2: Hybrid Icon Picker (Lucide Search + Gemini AI) */}
                <div className="p-3.5 sm:p-4 rounded-2xl bg-[var(--cat-card-bg)] border border-[var(--cat-card-border)] space-y-3">
                    <div className="flex items-center justify-between pb-1 border-b border-gray-100 dark:border-white/[0.06]">
                        <div className="flex items-center gap-1.5">
                            <Sparkles className="w-3.5 h-3.5 text-indigo-500" />
                            <h2 className="text-xs font-bold uppercase tracking-wider text-gray-800 dark:text-gray-200">
                                Choose Icon (Lucide & AI Search)
                            </h2>
                        </div>
                        <span className="text-[11px] text-gray-400 dark:text-gray-500">
                            Selected: <strong className="text-indigo-600 dark:text-indigo-400 font-mono">{iconName}</strong>
                        </span>
                    </div>
                    ...
                    <div className="space-y-2 pt-1">
                        ...
                        <div className="flex flex-row items-center gap-2 w-full">
                            ...
                            <button
                                ...
                                className="w-auto shrink-0 whitespace-nowrap px-3 sm:px-4 py-1.5 text-xs font-bold text-white bg-amber-600 hover:bg-amber-700 active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed rounded-xl transition-all flex items-center justify-center gap-1.5 h-[38px] sm:h-[34px] cursor-pointer"
                            >
                                {isReassigning ? (
                                    <Loader className="w-3.5 h-3.5 animate-spin" />
                                ) : (
                ...
                {!isNew && (
                    <div className="pt-2 pb-0 space-y-2">
                        <div className="flex items-center justify-between px-1">
                            ...
                            {matchingTransactions.length > 25 && (
                                <span className="text-[11px] text-gray-400 dark:text-gray-500">
                                    Showing latest 25
                                </span>
                            )}
                        ...
                        {matchingTransactions.length === 0 ? (
                            <div className="-mx-3.5 sm:-mx-6 lg:-mx-8 border-t border-gray-200/70 dark:border-white/[0.08] py-8 text-center px-4 bg-gray-50/40 dark:bg-white/[0.01]">
                                <History className="w-6 h-6 mx-auto mb-1.5 text-gray-400 dark:text-gray-500 opacity-60" />
                                <p className="text-xs text-gray-500 dark:text-gray-400">
                                    No transactions recorded under this category yet.
                                </p>
                            </div>
                        ) : (
                            <div className="-mx-3.5 sm:-mx-6 lg:-mx-8 border-t border-gray-200/70 dark:border-white/[0.08] divide-y divide-gray-100 dark:divide-white/[0.06]">
                                {matchingTransactions.slice(0, 25).map((t, idx) => {
                                    ...
                                    return (
                                        <div
                                            key={t.id || idx}
                                            className="px-4 sm:px-6 lg:px-8 py-2.5 sm:py-3 flex items-center justify-between gap-3 text-xs hover:bg-gray-50/80 dark:hover:bg-white/[0.03] transition-colors duration-150 group"
                                        >
                                            <div className="flex items-center gap-3 min-w-0 flex-1">
                                                {/* Pure clean icon with color tint (No background highlight box) */}
                                                <div className={`shrink-0 flex items-center justify-center ${
                                                    ...
                                                    <LiveIconComponent className="w-4 h-4 sm:w-5 sm:h-5" />
                                                </div>

                                                <div className="min-w-0 flex flex-col gap-0.5 flex-1 pr-2">
                                                    <p className="font-bold truncate text-xs sm:text-sm leading-tight text-gray-900 dark:text-white">
                                                        {t.description || 'Transaction'}
                                                    </p>
                                                    <div className="flex items-center gap-1.5 text-xs lg:text-[10px] font-medium text-gray-500 dark:text-gray-400 flex-wrap">
                                                        <span className="capitalize truncate max-w-[100px]">{t.category || label}</span>
                                                        {t.payment_method && (
                                                            <>
                                                                <span className="w-1 h-1 bg-gray-300 dark:bg-gray-600 rounded-full shrink-0"></span>
                                                                <span className="opacity-70 truncate max-w-[100px] uppercase text-[10px]">{t.payment_method}</span>
                                                            </>
                                                        )}
                                                        {t.source && (
                                                            <>
                                                                <span className="w-1 h-1 bg-gray-300 dark:bg-gray-600 rounded-full shrink-0"></span>
                                                                <span className="opacity-60 text-[9px] uppercase tracking-wider">{t.source}</span>
                                                            </>
                                                        )}
                                                    </div>
                                                </div>
