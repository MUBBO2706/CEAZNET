...
import { fetchImageProxy } from '../services/imageCachingService';
...
const fetchAndProcessColors = (imageUrl: string): Promise<DynamicColors> => {
    ...
    const promise = new Promise<DynamicColors>((resolve) => {
        ...
        // Fetch proxied image and process its dominant colors, sharing the active request promise if any
        fetchImageProxy(imageUrl)
            .then(dataUrl => {
                if (dataUrl) {
                    processImageFromSrc(dataUrl);
                } else {
                    processImageFromSrc(imageUrl);
                }
            })
            .catch(error => {
                console.warn(`Image proxy failed for ${imageUrl}, falling back to direct load. Error:`, error);
                processImageFromSrc(imageUrl);
            });
    });
